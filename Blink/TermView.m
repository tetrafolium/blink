...
- (id)initWithFrame:(CGRect)frame {
  ...
  _jsQueue =
      dispatch_queue_create(@"TermView.js".UTF8String, DISPATCH_QUEUE_SERIAL);
...
- (void)layoutSubviews {
  ...
  if (CGRectEqualToRect(_currentBounds, self.bounds) &&
      UIEdgeInsetsEqualToEdgeInsets(_currentAdditionalInsets,
                                    self.additionalInsets)) {
    return;
  }

  __weak typeof(self) weakSelf = self;
  _layoutDebounceTimer =
...
- (void)_addWebView {
  ...
  _webView = [[SmarterTermInput alloc] initWithFrame:[self webViewFrame]
                                       configuration:configuration];

  _gestureInteraction = [[WKWebViewGesturesInteraction alloc]
      initWithJsScrollerPath:@"t.scrollPort_.scroller_"];
...
- (void)loadWith:(MCPParams *)params;
{
  [_webView.configuration.userContentController
      addUserScript:[self _termInitScriptWith:params]];
...
- (void)setCursorBlink:(BOOL)state {
  [_webView evaluateJavaScript:term_setCursorBlink(state)
             completionHandler:nil];
}

- (void)setBoldAsBright:(BOOL)state {
  [_webView evaluateJavaScript:term_setBoldAsBright(state)
             completionHandler:nil];
}

- (void)setBoldEnabled:(NSUInteger)state {
  [_webView evaluateJavaScript:term_setBoldEnabled(state)
             completionHandler:nil];
}
...
- (void)reportTouchInPoint:(CGPoint)point {
  [_webView evaluateJavaScript:term_reportTouchInPoint(point)
             completionHandler:nil];
}
...
- (void)_evalJSScript:(NSString *)jsScript {
  dispatch_async(dispatch_get_main_queue(), ^{
    [_webView evaluateJavaScript:jsScript
...
//  Since TermView is a WKScriptMessageHandler, it must implement the
//  userContentController:didReceiveScriptMessage method. This is the method
//  that is triggered each time 'interOp' is sent a message from the JavaScript
//  code.
...
- (void)_onTerminalReady:(NSDictionary *)data {
  ...
  if (bgColor && bgColor.count == 3) {
    ...
    _gestureInteraction.indicatorStyle = color.isLight
  ...
  [UIView transitionFromView:_coverView
                      toView:_webView
                    duration:0.3
...
- (void)_handleSelectionChange:(NSDictionary *)data {
  ...
  [items
      addObject:[[UIMenuItem alloc] initWithTitle:@"Paste selection"
  ...
  if (_detectedLink) {
    ...
    [items
        addObject:[[UIMenuItem alloc]
                      initWithTitle:[@"Copy " stringByAppendingString:urlName]
                             action:@selector(copyLink:)]];

    NSString *actionTitle = [NSString
        stringWithFormat:@"%@ %@",
                         [self _menuActionTitleFromNSURL:_detectedLink],
                         urlName];
    [items addObject:[[UIMenuItem alloc] initWithTitle:actionTitle
                                                action:@selector(openLink:)]];
  } else {
    [items addObject:[[UIMenuItem alloc]
                         initWithTitle:@"Search"
                                action:@selector(googleSelection:)]];
  }
  [items
      addObject:[[UIMenuItem alloc] initWithTitle:@"Share"
...
- (void)modifySideOfSelection {
  [_webView evaluateJavaScript:term_modifySideSelection()
             completionHandler:nil];
}

- (void)modifySelectionInDirection:(NSString *)direction
                       granularity:(NSString *)granularity {
  [_webView evaluateJavaScript:term_modifySelection(direction, granularity)
             completionHandler:nil];
}

- (void)apiResponse:(NSString *)name response:(NSString *)response {
  [_webView evaluateJavaScript:term_apiResponse(name, response)
             completionHandler:nil];
}
...
- (NSURL *)_detectLinkInSelection:(NSDictionary *)data {
  ...
  [dataDetector
      enumerateMatchesInString:text
                    ...
                    usingBlock:^(NSTextCheckingResult *_Nullable res,
                      ...
                      if (url && res.range.location <= offset &&
                          res.range.location + res.range.length >= offset) {
                        result = url;
                        *stop = YES;
                      }
                    }];
...
- (WKUserScript *)_termInitScriptWith:(MCPParams *)params;
{
  ...
  BKFont *font =
      [BKFont withName:params.fontName ?: [BKDefaults selectedFontName]];
  ...
  {
    if (fontFamily) {
      [script addObject:term_setFontFamily(
                            fontFamily, font.systemWide ? @"dom" : @"canvas")];
    }

    [script addObject:term_setBoldEnabled(params.enableBold)];
    [script addObject:term_setBoldAsBright(params.boldAsBright)];

    NSString *themeContent = [[BKTheme
        withName:params.themeName ?: [BKDefaults selectedThemeName]] content];
    if (themeContent) {
      [script addObject:themeContent];
    }

    [script addObject:term_setFontSize(params.fontSize == 0
                                           ? [BKDefaults selectedFontSize]
                                           : @(params.fontSize))];
  ...
  return [[WKUserScript alloc]
        initWithSource:[script componentsJoinedByString:@"\n"]
         injectionTime:WKUserScriptInjectionTimeAtDocumentEnd
      forMainFrameOnly:YES];
}

- (void)applyTheme:(NSString *)themeName {
  NSString *themeContent =
      [[BKTheme withName:themeName ?: [BKDefaults selectedThemeName]] content];
  if (themeContent) {
    NSString *script =
        [NSString stringWithFormat:@"(function(){%@})();", themeContent];
...
- (void)terminate {
  ...
  [_webView.configuration.userContentController
      removeScriptMessageHandlerForName:@"interOp"];
}
