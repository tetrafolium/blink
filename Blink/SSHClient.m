...
@interface SSHClient (internal) <SSHClientConnectedChannelDelegate,
                                 SSHClientPortListenerDelegate>
- (int)_ssh_auth_fn_prompt:(const char *)prompt
                       buf:(char *)buf
                       len:(size_t)len
                      echo:(int)echo
                    verify:(int)verify;
...
int __ssh_auth_fn(const char *prompt, char *buf, size_t len, int echo,
                  int verify, void *userdata) {
  SSHClient *client = (__bridge SSHClient *)userdata;
  return [client _ssh_auth_fn_prompt:prompt
                                 buf:buf
                                 len:len
                                echo:echo
                              verify:verify];
...
- (void)sigwinch {
  ...
  [self _schedule:^{
    ...
    ssh_channel_change_pty_size(channel, _device->win.ws_col,
                                _device->win.ws_row);
  }];
}
...
- (int)_ssh_auth_fn_prompt:(const char *)prompt
  ...
  NSString *answer =
      [[self _getAnswersWithName:nil
                     instruction:nil
                      andPrompts:@[ @[ nsPrompt, @(echo) ] ]] firstObject];
  if (!answer) {
    return SSH_ERROR;
  }
...
- (NSArray<NSString *> *)_getAnswersWithName:(NSString *)name
                                 instruction:(NSString *)instruction
                                  andPrompts:(NSArray *)prompts {
  BOOL rawMode = _device.rawMode;
  //  [_device setRawMode:NO];

  if (name.length > 0) {
    name = [name stringByAppendingString:@"\n"];
    fwrite(name.UTF8String,
           [name lengthOfBytesUsingEncoding:NSUTF8StringEncoding], 1,
           _device.stream.out);
  }

  if (instruction.length > 0) {
    instruction = [instruction stringByAppendingString:@"\n"];
    fwrite(instruction.UTF8String,
           [instruction lengthOfBytesUsingEncoding:NSUTF8StringEncoding], 1,
           _device.stream.out);
  }
...
- (int)_connect {
  ...
  bool tcpKeepAlive =
      [_options[SSHOptionTCPKeepAlive] isEqual:SSHOptionValueYES];
  ...
  for (;;) {
    ...
    if (connectTimeout.integerValue > 0 &&
        -connectStart.timeIntervalSinceNow > connectTimeout.integerValue) {
    ...
    case SSH_ERROR: {
      ...
      if (error) {
        ...
        NSString *noCompressionOnServer = @"no match for method compression "
                                          @"algo client->server: server [none]";
        if ([_options[SSHOptionCompression] isEqual:SSHOptionValueYES] &&
            [error containsString:noCompressionOnServer]) {
          ssh_free(_session);
          [self _log_verbose:@"Server doesn't support compression. Connecting "
                             @"without compression\n"];
    ...
    case SSH_OK: {
      ...
      NSData *data =
          (__bridge_transfer NSData *)CFSocketCopyPeerAddress(sockRef);
      CFRelease(sockRef);
      ...
      if (data) {
        ...
        if (flags != tcpKeepAlive) {
          flags = tcpKeepAlive;
          [self _log_verbose:[NSString stringWithFormat:
                                           @"setting socket keepalive: %@\n",
                                           @(tcpKeepAlive)]];
          if (setsockopt(sock, SOL_SOCKET, SO_KEEPALIVE, (void *)&flags,
                         sizeof(flags))) {
            return SSH_ERROR;
          }
        }
        ...
        getnameinfo((const struct sockaddr *)[data bytes],
                    (socklen_t)data.length, host, sizeof(host), NULL, 0,
                    NI_NUMERICHOST);
        NSString *address = [NSString stringWithUTF8String:host];

        if (address && address.length &&
            [_options[SSHOptionPrintAddress] isEqual:SSHOptionValueYES]) {
          [self _log_info:[NSString
                              stringWithFormat:@"Connected to %@", address]];
        }
      }
...
- (int)_auth {
  ...
  // Who knows? we can success here too. See
  // https://github.com/blinksh/blink/issues/450
  ...
  BOOL optionPasswordAuth =
      [SSHOptionValueYES isEqual:_options[SSHOptionPasswordAuthentication]];
  BOOL optionKbdInteractiveAuth = [SSHOptionValueYES
      isEqual:_options[SSHOptionKbdInteractiveAuthentication]];
  BOOL optionPubKeyAuth =
      [SSHOptionValueYES isEqual:_options[SSHOptionPubkeyAuthentication]];
  BOOL optionIdenitiesOnly =
      [SSHOptionValueYES isEqual:_options[SSHOptionIdentitiesOnly]];
  int optionPasswordPromptsCount =
      [_options[SSHOptionNumberOfPasswordPrompts] intValue];
  ...
  for (int i = 0; i < maxPartialAuths; i++) {
    [self _log_verbose:[NSString stringWithFormat:
                                     @"using auth methods attempt: %@ of %@\n",
                                     @(i + 1), @(maxPartialAuths)]];

    // 3. get auth methods from server
    int methods = ssh_userauth_list(_session, NULL);

    // 4. user entered password in settings. So try to use it first to save
    // AuthTries
    if (optionPassword) {
      if (methods & SSH_AUTH_METHOD_PASSWORD && optionPasswordAuth) {
        ...
        if (rc == SSH_AUTH_SUCCESS) {
          ...
          continue;
                 ...
                 optionKbdInteractiveAuth) {
        rc = [self _auth_with_interactive_with_password:optionPassword
                                                prompts:1];
        if (rc == SSH_AUTH_SUCCESS) {
          return SSH_OK;
        } else if (rc == SSH_AUTH_PARTIAL) {
    ...
    if (methods & SSH_AUTH_METHOD_INTERACTIVE && optionKbdInteractiveAuth) {
      rc = [self
          _auth_with_interactive_with_password:optionPassword
                                       prompts:optionPasswordPromptsCount];
      if (rc == SSH_AUTH_SUCCESS) {
        return SSH_OK;
      } else if (rc == SSH_AUTH_PARTIAL) {
        continue;
    ...
    if (methods & SSH_AUTH_METHOD_PASSWORD && optionPasswordAuth) {
      ...
      rc = [self _auth_with_password:optionPassword
                             prompts:optionPasswordPromptsCount];
      if (rc == SSH_AUTH_SUCCESS) {
        return SSH_OK;
      } else if (rc == SSH_AUTH_PARTIAL) {
  ...
  for (NSString *identityfile in identityfiles) {
    ...
    if (secureKey) {
      ...
      rc = ssh_pki_import_privkey_base64(
          secureKey.privateKey.UTF8String, NULL, /* TODO: get stored */
          __ssh_auth_fn, (__bridge void *)self, &pkey);

      NSString *identityFilePath =
          [[BlinkPaths ssh] stringByAppendingPathComponent:identityfile];
      if ([fileManager fileExistsAtPath:identityFilePath]) {
        [self
            _log_verbose:[NSString stringWithFormat:
                                       @"warning: key '%@' duplicate in SE and "
                                       @"file system. Using key from SE   \n",
                                       identityfile]];
      }
    } else {
      ...
      if (![fileManager fileExistsAtPath:identityFilePath]) {
        identityFilePath =
            [[BlinkPaths ssh] stringByAppendingPathComponent:identityFilePath];
      }
      if (![fileManager fileExistsAtPath:identityFilePath]) {
        [self
            _log_verbose:[NSString
                             stringWithFormat:@"warning: no key found: '%@' \n",
                                              identityfile]];
        continue;
      }

      [self
          _log_verbose:[NSString stringWithFormat:@"import key from file %@\n",
                                                  identityfile]];
      rc = ssh_pki_import_privkey_file(identityFilePath.UTF8String, NULL,
                                       __ssh_auth_fn, (__bridge void *)self,
                                       &pkey);
    }
    if (rc == SSH_ERROR) {
      continue;
...
- (int)_auth_with_password:(NSString *)password prompts:(int)promptsCount {
  ...
  for (;;) {
    ...
    if (!password) {
      ...
      password = [[self _getAnswersWithName:NULL
                                instruction:NULL
                                 andPrompts:prompts] firstObject];
    }
...
- (int)_auth_with_interactive_with_password:(NSString *)password
                                    prompts:(int)promptsCount {
  ...
  for (;;) {
    ...
    case SSH_AUTH_INFO: {
      ...
      const char *instructionChars =
          ssh_userauth_kbdint_getinstruction(_session);
      ...
      NSMutableArray *prompts =
          [[NSMutableArray alloc] initWithCapacity:nprompts];
      ...
      if (password && nprompts == 1 &&
          [@"Password: " isEqual:[[prompts firstObject] firstObject]]) {
        answers = @[ password ];
      } else {
        answers = [self _getAnswersWithName:name
      ...
      for (int i = 0; i < answers.count; i++) {
        int rc =
            ssh_userauth_kbdint_setanswer(_session, i, [answers[i] UTF8String]);
        if (rc < 0) {
          return SSH_AUTH_ERROR;
        }
      }
...
- (NSString *)_pubkeyFingerPrint:(ssh_key)ssh_key {
  ...
  size_t hlen;
  int rc =
      ssh_get_publickey_hash(ssh_key, SSH_PUBLICKEY_HASH_SHA256, &hash, &hlen);
  if (rc < 0) {
    return nil;
  }

  char *fingerprint =
      ssh_get_fingerprint_hash(SSH_PUBLICKEY_HASH_SHA256, hash, hlen);
...
- (int)_verify_known_host {
  ...
  NSString *fingerprintMsg = [NSString
      stringWithFormat:@"%@ key fingerprint is %@.",
                       [self _keyTypeNameForKey:srv_pubkey], fingerprint];
  ...
  if (state == SSH_KNOWN_HOSTS_OTHER) {
    [self _log_verbose:@"The host key for this server was not found but an "
                       @"other type of key exists.\n"];
    [self _log_verbose:@"An attacker might change the default server key to "
                       @"confuse your client\n"];
  ...
  case SSH_KNOWN_HOSTS_OTHER:
    [self _device_log_info:@"The host key for this server was not found but an "
                           @"other type of key exists."];
    [self _device_log_info:@"An attacker might change the default server key "
  ...
  case SSH_KNOWN_HOSTS_UNKNOWN: {
    ...
    NSNumber *doEcho = @(YES);
    NSString *answer =
        [[[self _getAnswersWithName:@""
                        instruction:@"The server is unknown."
                         andPrompts:@[
                           @[ @"Do you trust the host key? (yes/no): ", doEcho ]
    ...
    answer = [[[self
        _getAnswersWithName:@""
                instruction:
                 ...
                 andPrompts:@[ @[ @"Do you agree? (yes/no): ", doEcho ] ]]
        firstObject] lowercaseString];
...
- (int)_open_channels {
  ...
  for (NSString *address in _options[SSHOptionLocalForward]) {
    ...
    if (rc != SSH_OK &&
        [SSHOptionValueYES isEqual:_options[SSHOptionExitOnForwardFailure]]) {
      return [self _exitWithCode:rc];
    }
  }
...
- (int)_request_pty:(ssh_channel)channel {
  ...
  for (;;) {
    ...
    rc = ssh_channel_request_pty_size(channel, term, _device->win.ws_col,
                                      _device->win.ws_row);
    switch (rc) {
    case SSH_AGAIN:
      [self _poll];
...
- (int)_start_session_channel {
  ...
  BOOL doRequestPTY =
      [_options[SSHOptionRequestTTY] isEqual:SSHOptionValueYES] ||
  ...
  _sessionChannel = [SSHClientConnectedChannel connect:channel
                                              withFdIn:_fdIn
                                                 fdOut:_fdOut
                                                 fdErr:_fdErr];
...
- (void)_ssh_send_env:(ssh_channel)channel {
  ...
  for (NSString *varName in vars) {
    [self
        _log_verbose:[NSString stringWithFormat:@"Sending env '%@'", varName]];
...
- (int)_start_stdio_forwarding:(NSString *)hostPort {
  ...
  if (hostAndPort.count != 2) {
    NSString *errorMessage =
        [NSString stringWithFormat:@"Bad stdio forwarding specification: %@",
                                   hostAndPort];
  ...
  for (;;) {
    ...
    int rc =
        ssh_channel_open_forward(channel, host.UTF8String, port, "stdio", port);
    switch (rc) {
    case SSH_OK:
      break;
    case SSH_AGAIN:
...
- (void)sshClientPortListener:(SSHClientPortListener *)listener
  ...
  for (;;) {
    ...
    int rc = ssh_channel_open_forward(
        channel, listener.remotehost.UTF8String, listener.remoteport,
        listener.sourcehost.UTF8String, listener.localport);
    switch (rc) {
    case SSH_OK:
      break;
    case SSH_AGAIN:
  ...
  SSHClientConnectedChannel *connectedChannel =
      [SSHClientConnectedChannel connect:channel withSocket:socket];
...
- (int)_start_listen_direct_forward:(NSString *)strAddress {
  SSHClientPortListener *portListener =
      [[SSHClientPortListener alloc] initInitWithAddress:strAddress];
...
- (int)_start_listen_reverse_forward:(NSString *)strAddress {
  ...
  NSMutableArray<NSString *> *parts =
      [[strAddress componentsSeparatedByString:@":"] mutableCopy];
  ...
  for (;;) {
    ...
    int rc = ssh_channel_listen_forward(_session, remotehost.UTF8String,
...
- (int)_connect_channel:(ssh_channel)channel to_port:(int)port {
  ...
  SSHClientConnectedChannel *connectedChannel =
      [SSHClientConnectedChannel connect:channel withSocket:sock];
  connectedChannel.delegate = self;
  [_connectedChannels addObject:connectedChannel];
  return SSH_OK;
}
...
- (void)_log_error {
  ...
  NSString *error =
      [[NSString alloc] initWithUTF8String:ssh_get_error(_session)];
  if (error.length == 0) {
    return;
  }
  __write(_fdErr, @"\r\n");
...
- (void)_start_server_keepalive_timer {
  ...
  [self
      _log_verbose:[NSString stringWithFormat:@"starting server keepalive: %@s",
                                              @(seconds)]];
  _serverKeepAliveTimer =
      [NSTimer timerWithTimeInterval:seconds
                              target:self
                            selector:@selector(_on_server_keep_alive)
                            userInfo:nil
                             repeats:YES];
...
- (ssh_session)_configured_session {
  ...
  _ssh_callbacks.channel_open_request_auth_agent_function =
      __ssh_channel_open_request_auth_agent_callback;
