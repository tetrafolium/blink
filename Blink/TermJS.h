...
NSString *_encodeString(NSString *str) {
  NSData *jsonData =
      [NSJSONSerialization dataWithJSONObject:str
...
NSString *term_init(BOOL accessibilityEnabled) {
  return [NSString stringWithFormat:@"term_init(%@);",
...
NSString *term_write(NSString *data) {
  ...
  NSMutableData *result =
      [[NSMutableData alloc] initWithCapacity:jsonData.length + 11 + 2];
...
NSString *term_writeB64(NSData *data) {
  return [NSString
      stringWithFormat:@"term_write_b64(\"%@\");",
...
NSString *term_clear() { return @"term_clear();"; }

NSString *term_reset() { return @"term_reset();"; }

NSString *term_focus() { return @"term_focus();"; }

NSString *term_blur() { return @"term_blur();"; }

NSString *term_reportTouchInPoint(CGPoint point) {
  return [NSString stringWithFormat:@"term_reportTouchInPoint(%@, %@);",
                                    @(point.x), @(point.y)];
}
...
NSString *term_increaseFontSize() { return @"term_increaseFontSize();"; }

NSString *term_decreaseFontSize() { return @"term_decreaseFontSize();"; }

NSString *term_resetFontSize() { return @"term_resetFontSize();"; }
...
NSString *term_getCurrentSelection() { return @"term_getCurrentSelection();"; }

NSString *term_setCursorBlink(BOOL state) {
  return [NSString stringWithFormat:@"term_set('cursor-blink', %@);",
                                    state ? @"true" : @"false"];
}

NSString *term_setBoldAsBright(BOOL state) {
  return [NSString stringWithFormat:@"term_set('enable-bold-as-bright', %@);",
                                    state ? @"true" : @"false"];
}
...
NSString *term_setFontFamily(NSString *family,
                             NSString *fontSizeDetectionMethod) {
  return [NSString stringWithFormat:@"term_setFontFamily(%@, %@);",
                                    _encodeString(family),
                                    _encodeString(fontSizeDetectionMethod)];
}

NSString *term_appendUserCss(NSString *css) {
  return
      [NSString stringWithFormat:@"term_appendUserCss(%@)", _encodeString(css)];
}

NSString *term_cleanSelection() { return @"term_cleanSelection();"; }

NSString *term_modifySelection(NSString *direction, NSString *granularity) {
  return [NSString stringWithFormat:@"term_modifySelection(%@, %@);",
                                    _encodeString(direction),
                                    _encodeString(granularity)];
}

NSString *term_modifySideSelection() { return @"term_modifySideSelection();"; }
...
NSString *term_displayInput(NSString *str, BOOL display) {
  return [NSString stringWithFormat:@"term_displayInput(%@, %@);",
                                    _encodeString(str),
                                    display ? @"true" : @"false"];
}
...
NSString *term_apiResponse(NSString *name, NSString *response) {
  return
      [NSString stringWithFormat:@"term_apiResponse(%@, %@);",
                                 _encodeString(name), _encodeString(response)];
}

NSString *term_restore() { return @"term_restore();"; }
