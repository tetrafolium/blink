...
+ (NSString *)documents {
  if (__documentsPath == nil) {
    __documentsPath = [NSSearchPathForDirectoriesInDomains(
...
+ (NSString *)iCloudDriveDocuments {
  if (__iCloudsDriveDocumentsPath == nil) {
    ...
    NSString *path = [[fm URLForUbiquityContainerIdentifier:
                              @"iCloud.com.carloscabanero.blinkshell"]
...
+ (void)linkICloudDriveIfNeeded {
  NSFileManager *fm = [NSFileManager defaultManager];
  NSString *icloudPath =
      [[self documents] stringByAppendingPathComponent:@"iCloud"];
  ...
  if (![fm createSymbolicLinkAtPath:icloudPath
                withDestinationPath:[self iCloudDriveDocuments]
                              error:&error]) {
    NSLog(@"Error: %@", error);
  };
}
...
+ (NSString *)blink {
  NSString *dotBlink =
      [[self documents] stringByAppendingPathComponent:@".blink"];
  ...
  [fileManager createDirectoryAtPath:dotBlink
         withIntermediateDirectories:YES
                          attributes:@{}
                               error:nil];
  return dotBlink;
...
+ (NSString *)ssh {
  ...
  [fileManager createDirectoryAtPath:dotSSH
         withIntermediateDirectories:YES
                          attributes:@{}
                               error:nil];
  return dotSSH;
