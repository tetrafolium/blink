...
- (NSArray<NSString *> *)pickWithInController:(UIViewController *)ctrl {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    pickerCtrl = [[UIDocumentPickerViewController alloc]
...
int link_files_main(int argc, char *argv[]) {
  ...
  if ([arg isEqualToString:@"-h"] || [arg isEqualToString:@"--help"]) {
    NSString *usage =
        [@[ @"usage: link-files [dest]" ] componentsJoinedByString:@"\n"];
    fputs(usage.UTF8String, thread_stdout);
    fputs("\n", thread_stderr);
    return 0;
  }
  ...
  if (@available(iOS 11.0, *)) {
    ...
    NSArray<NSString *> *folders =
        [picker pickWithInController:session.device.delegate.viewController];
    ...
    for (NSString *path in folders) {
      ...
      arg = nil;
      NSString *linkPath =
          [documentsPath stringByAppendingPathComponent:linkName];
      NSError *error = nil;
      if (![fm createSymbolicLinkAtPath:linkPath
                    withDestinationPath:path
                                  error:&error]) {
        fputs([NSString
                  stringWithFormat:@"Can't create new symbolic link at ~/%@:\n",
                                   linkName]
                  .UTF8String,
              thread_stderr);
      }
