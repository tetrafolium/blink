 ...
 */
static int rsa_hash_id_from_ident(const char *ident) {
...
static int rsa_hash_alg_nid(int type) {
  switch (type) {
  case SSH_DIGEST_SHA1:
    return NID_sha1;
  case SSH_DIGEST_SHA256:
...
int ssh_rsa_complete_crt_parameters(struct sshkey *key, const BIGNUM *iqmp) {
  ...
  if ((aux = BN_new()) == NULL || (rsa_dmq1 = BN_new()) == NULL ||
...
int ssh_rsa_verify(const struct sshkey *key, const u_char *sig, size_t siglen,
  ...
  if (key == NULL || key->rsa == NULL ||
      sshkey_type_plain(key->type) != KEY_RSA || sig == NULL || siglen == 0)
    return SSH_ERR_INVALID_ARGUMENT;
  RSA_get0_key(key->rsa, &rsa_n, NULL, NULL);
  if (BN_num_bits(rsa_n) < SSH_RSA_MINIMUM_MODULUS_SIZE)
    return SSH_ERR_KEY_LENGTH;
...
static int openssh_RSA_verify(int hash_alg, u_char *hash, size_t hashlen,
  ...
  if (rsasize <= 0 || rsasize > SSHBUF_MAX_BIGNUM || siglen == 0 ||
      siglen > rsasize) {
    ret = SSH_ERR_INVALID_ARGUMENT;
    goto done;
  }
