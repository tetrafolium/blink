...
- (instancetype)init {
  ...
  if (self) {
    ...
    _xSuccessURL = [NSURL
        URLWithString:[NSString stringWithFormat:@"blinkshell://x-success/%@",
                                                 _callID]];
    _xErrorURL = [NSURL
        URLWithString:[NSString stringWithFormat:@"blinkshell://x-error/%@",
                                                 _callID]];
    _xCancelURL = [NSURL
        URLWithString:[NSString stringWithFormat:@"blinkshell://x-cancel/%@",
                                                 _callID]];
  }
  return self;
}
...
- (int)run {
  ...
  NSURLComponents *components = [NSURLComponents componentsWithURL:_xURL
                                           resolvingAgainstBaseURL:YES];
  NSArray<NSURLQueryItem *> *originalQueryItems =
      [components queryItems] ?: @[];
  NSMutableArray<NSURLQueryItem *> *originalItemsWithParams =
      [originalQueryItems mutableCopy];

  for (NSArray *pair in _encodeInputParams) {
    NSURLQueryItem *item = [NSURLQueryItem queryItemWithName:pair.firstObject
                                                       value:pair.lastObject];
  ...
  if (!_async) {
    [newQueryItems
        addObject:[NSURLQueryItem
                      queryItemWithName:@"x-success"
                                  value:_xSuccessURL.absoluteString]];
    [newQueryItems
        addObject:[NSURLQueryItem queryItemWithName:@"x-error"
                                              value:_xErrorURL.absoluteString]];
    [newQueryItems addObject:[NSURLQueryItem
                                 queryItemWithName:@"x-cancel"
                                             value:_xCancelURL.absoluteString]];
  }
  ...
  NSURLComponents *comps = [NSURLComponents componentsWithURL:_xCallbackURL
                                      resolvingAgainstBaseURL:YES];
  ...
  if (originalCallbackURL) {
    ...
    NSURLComponents *originalComps =
        [NSURLComponents componentsWithURL:originalCallbackURL
                   resolvingAgainstBaseURL:YES];
...
void blink_handle_url(NSURL *url) {
  ...
  BOOL isValidXType = [xType isEqual:@"x-success"] ||
                      [xType isEqual:@"x-error"] || [xType isEqual:@"x-cancel"];
  if (!isValidXType) {
    return;
  }
  ...
  for (;;) {
    ...
    case 'p': {
      NSMutableArray *parts =
          [[@(thread_optarg) componentsSeparatedByString:@"="] mutableCopy];
