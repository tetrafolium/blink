...
static void usage(char *progname) {
  fprintf(thread_stderr,
          "Usage: %s -s TCP-port [-r] [-v] UDP-addr/UDP-port[/ttl]\n",
          progname);
  fprintf(
      thread_stderr,
      "    or %s -c TCP-addr[/TCP-port] [-r] [-v] UDP-addr/UDP-port[/ttl]\n",
      progname);
  fprintf(thread_stderr,
          "     -s: Server mode.  Wait for TCP connections on the port.\n");
  fprintf(thread_stderr,
          "     -c: Client mode.  Connect to the given address.\n");
  fprintf(thread_stderr, "     -r: RTP mode.  Connect/listen on ports N and "
                         "N+1 for both UDP and TCP.\n");
  fprintf(thread_stderr, "         Port numbers must be even.\n");
  fprintf(thread_stderr, "     -v: Verbose mode.  Specify -v multiple times "
...
static void setup_udp_recv(struct relay *relay) {
  ...
  opt = 1;
  if (setsockopt(relay->udp_recv_sock, SOL_SOCKET, SO_REUSEADDR, (void *)&opt,
                 sizeof(opt)) < 0) {
...
#ifdef SO_REUSEPORT
  opt = 1;
  if (setsockopt(relay->udp_recv_sock, SOL_SOCKET, SO_REUSEPORT, (void *)&opt,
                 sizeof(opt)) < 0) {
...
static void setup_server_listen(struct relay *relay) {
  ...
  opt = 1;
  if (setsockopt(relay->tcp_listen_sock, SOL_SOCKET, SO_REUSEADDR, (void *)&opt,
                 sizeof(opt)) < 0) {
...
#ifdef SO_REUSEPORT
  opt = 1;
  if (setsockopt(relay->tcp_listen_sock, SOL_SOCKET, SO_REUSEPORT, (void *)&opt,
                 sizeof(opt)) < 0) {
  ...
  if (debug)
    fprintf(thread_stderr, "Listening for TCP connections on port %hu\n",
...
static void setup_tcp_client(struct relay *relay) {
  ...
  if (debug)
    fprintf(thread_stderr, "Connected TCP to %s/%hu\n",
            inet_ntoa(relay->tcpaddr.sin_addr), ntohs(relay->tcpaddr.sin_port));
} /* connect_tcp */
...
static int udp_to_tcp(struct relay *relay) {
  ...
  if ((buflen = recvfrom(relay->udp_recv_sock, p.buf, UDPBUFFERSIZE, 0,
                         (struct sockaddr *)&remote_udpaddr, &addrlen)) <= 0) {
    if (buflen < 0) {
  ...
  if (debug > 1) {
    fprintf(thread_stderr, "Received %d byte UDP packet from %s/%hu\n", buflen,
            inet_ntoa(remote_udpaddr.sin_addr), ntohs(remote_udpaddr.sin_port));
  }
...
static int tcp_to_udp(struct relay *relay) {
  ...
  if (debug > 1) {
    fprintf(thread_stderr,
            "Received packet on TCP, length %u; sending as UDP\n",
            relay->packet_length);
  }
  if (send(relay->udp_send_sock, relay->packet_start, relay->packet_length, 0) <
      0) {
    if (errno != ECONNREFUSED) {
      ...
      if (getsockopt(relay->udp_send_sock, SOL_SOCKET, SO_ERROR, (void *)&err,
                     &len) < 0) {
