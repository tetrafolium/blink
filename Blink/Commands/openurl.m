...
NSURL *__blink_browser_app_url(NSURL *srcURL) {
  ...
  if (([browser isEqualToString:@"firefox"]) ||
    ...
    NSString *url =
        [absSrcURLStr stringByAddingPercentEncodingWithAllowedCharacters:
  ...
  NSString *browserAppUrlStr =
      [absSrcURLStr stringByReplacingCharactersInRange:NSMakeRange(0, 4)
                                            withString:browser];
...
int blink_openurl_main(int argc, char *argv[]) {
  NSString *usage = [@[
    ...
    [NSString stringWithFormat:@"  %@", [__blink_known_browsers()
