...
void __write_data(NSMutableData *data, NSOutputStream *output) {
  ...
  [data
      enumerateByteRangesUsingBlock:^(const void *_Nonnull bytes,
                                      NSRange byteRange, BOOL *_Nonnull stop) {
...
NSInteger __write_buffer(const uint8_t *buffer, NSUInteger len,
                         NSMutableData *data, NSOutputStream *output) {
  if (data.length == 0) {
    if (len == 0) {
      return len;
    }
...
void __write_channel(ssh_channel channel, NSMutableData *data,
...
- (int)pairChannel:(ssh_channel)channel
...
+ (nullable SSHClientConnectedChannel *)connect:(ssh_channel)channel
  ...
  int rc = [connector pairChannel:channel
                         withFdIn:fdIn
                            fdOut:fdOut
                            fdErr:fdErr];
  if (rc == SSH_OK) {
...
static int __stream_connector_channel_data_cb(ssh_session session,
                                              ssh_channel channel, void *data,
                                              uint32_t len, int is_stderr,
                                              StreamConnector *connector) {
  if (connector->_channel_flags == SSH_CONNECTOR_BOTH) {
    return (int)__write_buffer(data, len, connector->_outputData,
                               connector->_outputStream);
  }
  return 0;
}

void __stream_connector_channel_eof_cb(ssh_session session, ssh_channel channel,
                                       StreamConnector *connector) {
  ssh_channel_close(channel);
}

void __stream_connector_channel_close_cb(ssh_session session,
                                         ssh_channel channel,
                                         StreamConnector *connector) {
  [connector close];
}
...
- (int)pairChannel:(ssh_channel)channel withSocket:(dispatch_fd_t)socket {
  ...
  _channel_cb.channel_exit_status_function =
      __stream_connector_channel_exit_status_cb;
...
void __sock_connector_channel_eof_cb(ssh_session session, ssh_channel channel,
                                     SockConnector *connector) {
  ssh_channel_close(channel);
}

void __sock_connector_channel_close_cb(ssh_session session, ssh_channel channel,
                                       SockConnector *connector) {
  [connector close];
}
...
void __sock_callback(CFSocketRef s, CFSocketCallBackType type,
    ...
    for (;;) {
      ...
      if (len == 0) { // EOF
        if (connector->_inputData.length > 0) {
          __write_channel(connector->_channel, connector->_inputData,
                          SSH_CONNECTOR_STDOUT);
        }
        ssh_channel_send_eof(connector->_channel);
        //          CFRunLoopRemoveSource(CFRunLoopGetCurrent(),
        //          connector->_sourceRef, kCFRunLoopDefaultMode);
        return;
      } else if (len < 0) {
        // try again
        break;
...
- (int)pairChannel:(ssh_channel)channel
  ...
  _socketRef = CFSocketCreateWithNative(NULL, fdIn, kCFSocketReadCallBack,
                                        __sock_callback, &ctx);
  ...
  _channel_cb.channel_exit_status_function =
      __sock_connector_channel_exit_status_cb;
