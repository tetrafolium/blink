...
void __on_pipebroken_signal(int signum) { NSLog(@"PIPE is broken"); }

void __setupProcessEnv() {
  NSBundle *mainBundle = [NSBundle mainBundle];
  int forceOverwrite = 1;
  NSString *SSL_CERT_FILE = [mainBundle pathForResource:@"cacert"
                                                 ofType:@"pem"];
  setenv("SSL_CERT_FILE", SSL_CERT_FILE.UTF8String, forceOverwrite);

  NSString *locales_path = [mainBundle pathForResource:@"locales"
                                                ofType:@"bundle"];
...
- (BOOL)application:(UIApplication *)application
  ...
  dispatch_async(bgQueue, ^{
    addCommandList([[NSBundle mainBundle]
        pathForResource:@"blinkCommandsDictionary"
                 ofType:@"plist"]); // Load blink commands to ios_system
    __setupProcessEnv();            // we should call this after ios_system
                         // initializeEnvironment to override its defaults.
  });
  ...
  //  [nc addObserver:self selector:@selector(_active)
  //  name:@"UIApplicationSystemNavigationActionChangedNotification"
  //  object:nil];
...
//  if ([n.name
//  isEqualToString:@"UIApplicationSystemNavigationActionChangedNotification"])
//  {
//    [[SmarterTermInput shared] realBecomeFirstResponder];
//  }
//
...
- (void)_loadProfileVars {
  ...
  NSString *profile =
      [NSString stringWithContentsOfFile:[BlinkPaths blinkProfileFile]
                                encoding:NSUTF8StringEncoding
                                   error:nil];
  [profile
      enumerateLinesUsingBlock:^(NSString *_Nonnull line, BOOL *_Nonnull stop) {
        NSMutableArray<NSString *> *parts =
            [[line componentsSeparatedByString:@"="] mutableCopy];
        if (parts.count < 2) {
          return;
        }

        NSString *varName =
            [parts.firstObject stringByTrimmingCharactersInSet:whiteSpace];
        if (varName.length == 0) {
          return;
        }
        [parts removeObjectAtIndex:0];
        NSString *varValue = [[parts componentsJoinedByString:@"="]
            stringByTrimmingCharactersInSet:whiteSpace];
        if ([varValue hasSuffix:@"\""] || [varValue hasPrefix:@"\""]) {
          NSData *data = [varValue dataUsingEncoding:NSUTF8StringEncoding];
          varValue =
              [varValue substringWithRange:NSMakeRange(1, varValue.length - 1)];
          if (data) {
...
- (void)_startMonitoringForSuspending {
  ...
  _suspendTaskId =
      [application beginBackgroundTaskWithName:@"Suspend"
                             expirationHandler:^{
  ...
  _suspendTimer =
      [NSTimer scheduledTimerWithTimeInterval:time
                                       target:self
                                     selector:@selector
                                     (_suspendApplicationWithSuspendTimer)
                                     userInfo:nil
                                      repeats:NO];
}
...
- (UISceneConfiguration *)application:(UIApplication *)application
  ...
  return
      [UISceneConfiguration configurationWithName:@"main"
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
  ...
  [ctrl moveToShellWithKey:response.notification.request.content
                               .threadIdentifier];
