...
NSDictionary *__locationToJson(CLLocation *location) {
  ...
  if (location.speed >= 0) {
    ...
    gps[(NSString *)kCGImagePropertyGPSSpeed] =
        @(location.speed * (3600.0 / 1000.0));
  }
...
- (void)_postLockNotifications {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:BLGeoLockNotification
                    object:nil];
  ...
  [center getNotificationSettingsWithCompletionHandler:^(
    ...
    content.body =
        @"Your device get out of geo range lock.\nAll session are terminated.";
    UNNotificationRequest *req =
        [UNNotificationRequest requestWithIdentifier:@"geo.lock"
                                             content:content
                                             trigger:nil];
...
- (void)locationManager:(CLLocationManager *)manager
  ...
  if (_lockDistanceInMeters) {
    ...
    if (_lockCenter) {
      CLLocationDistance distance =
          [_lockCenter distanceFromLocation:[_recentLocations lastObject]];
  ...
  if (_recentLocations.count > maxHistory) {
    [_recentLocations
        removeObjectsInRange:NSMakeRange(0,
                                         _recentLocations.count - maxHistory)];
  }
}
...
- (NSString *)currentJSON {
  ...
  NSData *data =
      [NSJSONSerialization dataWithJSONObject:__locationToJson(loc)
