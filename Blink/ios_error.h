...
extern int ios_executable(
    const char *cmd); // is this command part of the "shell" commands?
extern int ios_system(const char *inputCmd); // execute this command (executable
                                             // file or builtin command)
extern FILE *
ios_popen(const char *command,
          const char *type); // Execute this command and pipe the result
extern int ios_kill(void);   // kill the current running command

extern void
ios_exit(int errorCode) __dead2; // set error code and exits from the thread.
...
extern size_t ios_fwrite(const void *ptr, size_t size, size_t nitems,
                         FILE *stream);
