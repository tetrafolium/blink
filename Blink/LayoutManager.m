...
NSString *LayoutManagerBottomInsetDidUpdate =
    @"LayoutManagerBottomInsetDidUpdate";
...
+ (void)updateMainWindowKBBottomInset:(CGFloat)bottomInset {
  ...
  __debounceTimer = [NSTimer
      scheduledTimerWithTimeInterval:0.6
                             repeats:NO
...
+ (UIEdgeInsets)buildSafeInsetsForController:(UIViewController *)ctrl
                                     andMode:(BKLayoutMode)mode {
  ...
  UIEdgeInsets deviceMargins =
      window.safeAreaInsets; // UIEdgeInsetsZero;// ctrl.viewDeviceSafeMargins;

  BOOL fullScreen = CGRectEqualToRect(mainScreen.bounds, window.bounds);
  CGFloat slideOverVerticalMargin =
      (mainScreen.bounds.size.height - window.bounds.size.height) * 0.5;

  UIEdgeInsets result = UIEdgeInsetsZero;

  switch (mode) {
  case BKLayoutModeDefault:
    return [self buildSafeInsetsForController:ctrl
                                      andMode:[self deviceDefaultLayoutMode]];
