...
- (void)showNonModifierKeySection:(SKNonModifierButtonType)type {
  ...
  [selectedStackView.topAnchor
      constraintEqualToAnchor:_nonModifierScrollView.topAnchor]
      .active = YES;
  [selectedStackView.leadingAnchor
      constraintEqualToAnchor:_nonModifierScrollView.leadingAnchor]
      .active = YES;
  [selectedStackView.trailingAnchor
      constraintEqualToAnchor:_nonModifierScrollView.trailingAnchor]
      .active = YES;
  [selectedStackView.bottomAnchor
      constraintEqualToAnchor:_nonModifierScrollView.bottomAnchor]
      .active = YES;
  [selectedStackView.arrangedSubviews[0].widthAnchor
      constraintGreaterThanOrEqualToAnchor:_ctrlButton.widthAnchor
                                multiplier:0.8]
      .active = YES;
}
...
- (UIStackView *)smartKeysStackWith:(NSArray<SmartKey *> *)keys {
  ...
  for (SmartKey *key in keys) {
    SKNonModifierButton *button =
        [SKNonModifierButton buttonWithType:UIButtonTypeCustom];
    button.backgroundColor = [UIColor grayColor];
    [button setTitle:key.name forState:UIControlStateNormal];
    [stack addArrangedSubview:button];
    [button addTarget:nil
                  action:@selector(nonModifierUp:)
    ...
    [button addTarget:nil
                  action:@selector(nonModifierDown:)
...
// "Alternative" / "More" button -> Allows the delegate to decide how to show
// the other possible interfaces it has
