...
+ (id)sharedHandler {
  if ([BKUserConfigurationManager userSettingsValueForKey:BKUserConfigiCloud]) {
    ...
    // If user settings is turned off, return nil, so that all messages are
    // ignored
...
- (instancetype)init {
  ...
  if (self) {
    [[NSNotificationCenter defaultCenter]
        addObserver:self
           selector:@selector(checkForReachabilityAndSync:)
    ...
    [self loadSyncItems];
    CKDatabase *database =
        [[CKContainer containerWithIdentifier:BKiCloudContainerIdentifier]
            privateCloudDatabase];
    if (!database) {
      return nil;
    }
    CKRecordZone *zone =
        [[CKRecordZone alloc] initWithZoneName:BKiCloudZoneName];
    if (!zone) {
      return nil;
    }
    [database saveRecordZone:zone
           completionHandler:^(CKRecordZone *_Nullable zone,
...
- (void)loadSyncItems {
  // Load IDs from file
  if ((syncItems = [NSKeyedUnarchiver
           unarchiveObjectWithFile:[BlinkPaths blinkSyncItemsFile]]) == nil) {
...
+ (BOOL)saveSyncItems {
  return [NSKeyedArchiver archiveRootObject:syncItems
                                     toFile:[BlinkPaths blinkSyncItemsFile]];
}
...
- (void)deleteAllItems {
  NSMutableArray *deletedHosts = [NSMutableArray
      arrayWithArray:[syncItems objectForKey:BKiCloudSyncDeletedHosts]];
  ...
  NSMutableArray *deletedKeys = [NSMutableArray
      arrayWithArray:[syncItems objectForKey:BKiCloudSyncDeletedKeys]];
...
- (void)syncFromiCloud {
  ...
  CKDatabase *database =
      [[CKContainer containerWithIdentifier:BKiCloudContainerIdentifier]
          privateCloudDatabase];
  CKQuery *hostQuery =
      [[CKQuery alloc] initWithRecordType:@"BKHost"
  ...
  if ([BKUserConfigurationManager
          userSettingsValueForKey:BKUserConfigiCloudKeys]) {
    CKQuery *pubKeyQuery = [[CKQuery alloc]
        initWithRecordType:@"BKPubKey"
                 predicate:[NSPredicate predicateWithValue:YES]];
...
- (void)deleteRecord:(CKRecordID *)recordId
  ...
  [database deleteRecordWithID:recordId
             completionHandler:^(CKRecordID *_Nullable recordID,
                                 NSError *_Nullable error) {
               if (error) {
                 NSMutableArray *deletedItems = [NSMutableArray array];
                 if ([syncItems objectForKey:key]) {
                   deletedItems = [NSMutableArray
                       arrayWithArray:[syncItems objectForKey:key]];
                 }
...
- (void)createNewHost:(BKHosts *)host {
  ...
  [database saveRecord:hostRecord
      completionHandler:^(CKRecord *_Nullable record,
                          NSError *_Nullable error) {
        [BKHosts updateHost:host.host
                   withiCloudId:record.recordID
            andLastModifiedTime:record.modificationDate];
      }];
}
...
- (void)mergeHosts:(NSArray *)hostRecords {
  for (CKRecord *hostRecord in hostRecords) {
    if ([hostRecord valueForKey:@"host"]) {
      ...
      if (hosts) {
        if ([hosts.lastModifiedTime compare:hostRecord.modificationDate] ==
            NSOrderedDescending) {
  ...
  for (BKHosts *hosts in [BKHosts all]) {
    if (hosts.iCloudRecordId == nil &&
        (!hosts.iCloudConflictDetected ||
         hosts.iCloudConflictDetected == [NSNumber numberWithBool:NO])) {
      [self createNewHost:hosts];
    } else {
      NSLog(@"Conflict detected Hence not saving to iCloud");
      // Find items deleted from iCloud
      if ((!hosts.iCloudConflictDetected ||
           hosts.iCloudConflictDetected == [NSNumber numberWithBool:NO])) {
        NSPredicate *deletedPredicate = [NSPredicate
            predicateWithFormat:@"SELF.recordID.recordName contains %@",
                                hosts.iCloudRecordId.recordName];
        NSArray *filteredAray =
            [hostRecords filteredArrayUsingPredicate:deletedPredicate];
...
- (void)saveHostRecord:(CKRecord *)hostRecord withHost:(NSString *)host {
  ...
  if (moshPort && moshPortEnd) {
    moshPortRange = [NSString
        stringWithFormat:@"%@:%@", moshPortRange, moshPortEnd.stringValue];
  }
  ...
  [BKHosts updateHost:updatedHost.host
             withiCloudId:hostRecord.recordID
      andLastModifiedTime:hostRecord.modificationDate];
}
...
//  CKDatabase *database = [[CKContainer
//  containerWithIdentifier:@"iCloud.com.carloscabanero.blinkshell"]privateCloudDatabase];
//  CKRecord *keyRecord = [BKPubKey recordFromHost:key];
//  [database saveRecord:hostRecord completionHandler:^(CKRecord * _Nullable
//  record, NSError * _Nullable error) {
//    [BKHosts saveHost:host.host withiCloudId:record.recordID
//    andLastModifiedTime:record.modificationDate];
//  }];
//}
