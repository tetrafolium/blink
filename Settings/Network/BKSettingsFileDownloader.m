...
+ (void)downloadFileAtUrl:(NSString *)urlString
    withCompletionHandler:
        (void (^)(NSData *fileData, NSError *error))completionHandler {
  [self downloadFileAtUrl:urlString
          expectedMIMETypes:nil
  ...
  //     completionHandler:^(NSURL *location, NSURLResponse *response, NSError
  //     *error) {
  //       if (error.code != -999) {
...
+ (void)downloadFileAtUrl:(NSString *)urlString
        expectedMIMETypes:(NSArray *)mimeTypes
    withCompletionHandler:
        (void (^)(NSData *fileData, NSError *error))completionHandler {
  [BKSettingsFileDownloader cancelRunningDownloads];
      ...
      downloadTaskWithURL:url
        completionHandler:^(NSURL *location, NSURLResponse *response,
          ...
          if (!error && mimeTypes) {
            ...
            [mimeTypes enumerateObjectsUsingBlock:^(
                           NSString *type, NSUInteger idx, BOOL *stop) {
            ...
            if (!acceptedMIMEType) {
              NSString *msg =
                  [NSString stringWithFormat:@"Unsupported media type %@.",
                                             [response MIMEType]];
              error = [NSError errorWithDomain:@"BKSettingsErrorDomain"
                                          code:415
                                      userInfo:@{
                                        NSLocalizedDescriptionKey :
                                            NSLocalizedString(msg, nil)
                                      }];
            }
          }
          if (error.code != -999) {
