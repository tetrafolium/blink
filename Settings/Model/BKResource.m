...
- (NSString *)content {
  return [NSString stringWithContentsOfFile:self.fullPath
  ...
  if (!rscs) {
    ...
    NSArray *resourceFiles = [[NSFileManager defaultManager]
        ...
        includingPropertiesForKeys:properties
                           options:(NSDirectoryEnumerationSkipsHiddenFiles)error
                                  :&error];
    ...
    if (resourceFiles != nil) {
      for (NSURL *file in resourceFiles) {
        ...
        BKResource *res = [[self alloc]
            initWithName:[fileName stringByReplacingOccurrencesOfString:fileExt
                                                             withString:@""]
...
+ (NSMutableArray *)customResources {
  ...
  if (!rscs) {
    if ((rscs = [NSKeyedUnarchiver
             unarchiveObjectWithFile:self.customResourcesListLocation.path]) ==
        nil) {
      rscs = [[NSMutableArray alloc] init];
    }
...
+ (NSURL *)defaultResourcesLocation {
  return [[[NSBundle mainBundle] resourceURL]
      URLByAppendingPathComponent:[self resourcesPathName]];
}
...
+ (NSURL *)customResourcesListLocation {
  NSString *listFileName =
      [[self resourcesPathName] stringByAppendingString:@"List"];
...
+ (BOOL)saveAll {
  // Save IDs to file
  return
      [NSKeyedArchiver archiveRootObject:self.customResources
                                  toFile:self.customResourcesListLocation.path];
}
...
+ (instancetype)saveResource:(NSString *)name
  ...
  NSURL *filePath =
      [self.customResourcesLocation URLByAppendingPathComponent:fileName];
  [[NSFileManager defaultManager]
             createDirectoryAtURL:self.customResourcesLocation
  ...
  BKResource *res = [[self alloc] initWithName:name
                                   andFileName:fileName
                                         onURL:self.customResourcesLocation];
...
+ (void)removeResourceAtIndex:(int)index {
  [self.customResources
      removeObjectAtIndex:index - [self defaultResourcesCount]];
  [self saveAll];
}
