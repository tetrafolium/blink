...
NSString *__get_passphrase(UIViewController *ctrl) {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    UIAlertController *passphraseRequest = [UIAlertController
        alertControllerWithTitle:@"Encrypted key"
                         message:@"Please insert passphrase"
                  preferredStyle:UIAlertControllerStyleAlert];
    [passphraseRequest
        addTextFieldWithConfigurationHandler:^(UITextField *textField) {
          textField.placeholder =
              NSLocalizedString(@"Enter passphrase", @"Passphrase");
        ...
        [UIAlertAction actionWithTitle:@"OK"
                               ...
                               handler:^(UIAlertAction *_Nonnull action) {
                                 UITextField *passphrase =
                                     passphraseRequest.textFields.lastObject;
...
+ (void)importPrivateKey:(NSString *)privateKey
  ...
  dispatch_async(dispatch_get_global_queue(0, 0), ^{
    struct sshbuf *b = sshbuf_from(
        privateKey.UTF8String,
        [privateKey lengthOfBytesUsingEncoding:NSUTF8StringEncoding]);
    ...
    NSString *passphrase = @"";
    for (;;) {
      rcs = sshkey_parse_private_fileblob(b, passphrase.UTF8String, &keyp,
                                          &commepntp);
...
+ (NSArray<const NSString *> *)supportedKeyTypes {
  return
      @[ BK_KEYTYPE_DSA, BK_KEYTYPE_RSA, BK_KEYTYPE_ECDSA, BK_KEYTYPE_Ed25519 ];
}
...
- (NSString *)publicKeyWithComment:(NSString *)comment {
  ...
  NSString *commentedKey =
      [NSString stringWithFormat:@"%@ %@ %@", keyType, key, comment];
  return commentedKey;
}
...
+ (BOOL)saveIDS {
  // Save IDs to file
  return [NSKeyedArchiver archiveRootObject:Identities
                                     toFile:[BlinkPaths blinkKeysFile]];
}

+ (void)loadIDS {
  // Load IDs from file
  if ((Identities = [NSKeyedUnarchiver
           unarchiveObjectWithFile:[BlinkPaths blinkKeysFile]]) == nil) {
...
+ (id)saveCard:(NSString *)ID
  ...
  if (!card) {
    card = [[BKPubKey alloc] initWithID:ID
...
- (id)initWithID:(NSString *)ID
    privateKeyRef:(NSString *)privateKeyRef
        publicKey:(NSString *)publicKey {
  self = [self init];
  if (self == nil)
    return nil;
  ...
  if ([activityType isEqualToString:UIActivityTypeMail] ||
    ...
    NSURL *url =
        [NSURL fileURLWithPath:[NSTemporaryDirectory()
                                   stringByAppendingString:tempFilename]];
