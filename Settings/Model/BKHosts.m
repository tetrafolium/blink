...
- (id)initWithCoder:(NSCoder *)coder {
  ...
  _iCloudConflictDetected =
      [coder decodeObjectForKey:@"iCloudConflictDetected"];
...
- (void)encodeWithCoder:(NSCoder *)encoder {
  ...
  [encoder encodeObject:_iCloudConflictDetected
                 forKey:@"iCloudConflictDetected"];
  ...
  if (self) {
    ...
    if (![moshPortRange isEqualToString:@""]) {
      NSArray<NSString *> *parts =
          [moshPortRange componentsSeparatedByString:@":"];
...
+ (BOOL)saveHosts {
  // Save IDs to file
  return [NSKeyedArchiver archiveRootObject:Hosts
                                     toFile:[BlinkPaths blinkHostsFile]];
}
...
+ (instancetype)saveHost:(NSString *)host
  ...
  if (!bkHost) {
    bkHost = [[BKHosts alloc] initWithHost:newHost
                                  hostName:hostName
                                   sshPort:sshPort
                                      user:user
                               passwordRef:pwdRef
...
+ (void)loadHosts {
  // Load IDs from file
  if ((Hosts = [NSKeyedUnarchiver
           unarchiveObjectWithFile:[BlinkPaths blinkHostsFile]]) == nil) {
...
+ (NSMutableArray *)predictionStringList {
  return [NSMutableArray
      arrayWithObjects:@"Adaptive", @"Always", @"Never", @"Experimental", nil];
}
...
+ (BKHosts *)hostFromRecord:(CKRecord *)hostRecord {
  ...
  BKHosts *host = [[BKHosts alloc]
       initWithHost:[hostRecord valueForKey:@"host"]
           hostName:[hostRecord valueForKey:@"hostName"]
            sshPort:[hostRecord valueForKey:@"port"]
                        ? [[hostRecord valueForKey:@"port"] stringValue]
                        : @""
