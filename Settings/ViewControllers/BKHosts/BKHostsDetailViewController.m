...
- (void)viewDidLoad {
  ...
  if (_bkHost != nil) {
    ...
    _predictionDetail.text =
        [BKHosts predictionStringForRawValue:_bkHost.prediction.intValue];
    _moshServerField.text = _bkHost.moshServer;
    if (_bkHost.moshPort != nil) {
      if (_bkHost.moshPortEnd != nil) {
        _moshPortField.text = [NSString
            stringWithFormat:@"%@:%@", _bkHost.moshPort, _bkHost.moshPortEnd];
      } else {
        _moshPortField.text =
            [NSString stringWithFormat:@"%@", _bkHost.moshPort];
      }
    }
  ...
  [self.hostKeyDetail addObserver:self
                       forKeyPath:@"text"
                          options:0
                          context:nil];
  [self.predictionDetail addObserver:self
                          forKeyPath:@"text"
                             options:0
                             context:nil];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
  ...
  //   UITableView *predictionMode = (UITableView
  //   *)segue.destinationViewController.view; [predictionMode
  //   setDelegate:self];
  // }
...
- (BOOL)shouldPerformSegueWithIdentifier:(NSString *)identifier
  ...
  if ([identifier isEqualToString:@"unwindFromCreate"]) {
    // An existing host with same name should not exist, but while editing an
    // existing Host, it should not show error
    if ([BKHosts withHost:_hostField.text] &&
        ![_hostField.text isEqualToString:_bkHost.host]) {
      errorMsg = @"Cannot have two hosts with the same name.";
    } else if ([_hostField.text
                   rangeOfCharacterFromSet:[NSCharacterSet
                                               whitespaceCharacterSet]]
                   .location != NSNotFound) {
      errorMsg = @"Spaces are not permitted in the host.";
    } else if ([_hostNameField.text
                   rangeOfCharacterFromSet:[NSCharacterSet
                                               whitespaceCharacterSet]]
                   .location != NSNotFound) {
      errorMsg = @"Spaces are not permitted in the host name.";
    } else if ([_userField.text
                   rangeOfCharacterFromSet:[NSCharacterSet
                                               whitespaceCharacterSet]]
                   .location != NSNotFound) {
      ...
      _bkHost =
          [BKHosts saveHost:self.bkHost.host
              ...
              moshPortRange:_moshPortField.text
                 ...
                 prediction:[BKHosts
                                predictionValueForString:_predictionDetail.text]
                   proxyCmd:_proxyCmdField.text];
      [BKHosts updateHost:_bkHost.host
                 withiCloudId:_bkHost.iCloudRecordId
          andLastModifiedTime:[NSDate date]];
    ...
    if (errorMsg) {
      ...
      UIAlertController *alertController = [UIAlertController
          alertControllerWithTitle:@"Hosts error"
                           message:errorMsg
...
- (BOOL)textField:(UITextField *)textField
  ...
  if (textField == _sshPortField) {
    ...
    return ([string stringByTrimmingCharactersInSet:nonColumNonNumberSet]
                .length > 0) ||
           [string isEqualToString:@""];
  } else if (textField == _hostField || textField == _hostNameField ||
...
- (IBAction)textFieldDidChange:(id)sender {
  if (_hostField.text.length && _hostNameField.text.length &&
      !_bkHost.iCloudConflictDetected.boolValue && !_isConflictCopy) {
...
- (void)tableView:(UITableView *)tableView
  ...
  if (indexPath.section == 1) {
    if (indexPath.row == 0) {
      BKHostsDetailViewController *iCloudCopyViewController = [self.storyboard
          instantiateViewControllerWithIdentifier:@"createHost"];
      iCloudCopyViewController.isConflictCopy = YES;
      iCloudCopyViewController.bkHost = _bkHost.iCloudConflictCopy;
      [self.navigationController pushViewController:iCloudCopyViewController
                                           animated:YES];
    } else if (indexPath.row == 1) {
      if (_bkHost.iCloudRecordId) {
        [[BKiCloudSyncHandler sharedHandler]
            deleteRecord:_bkHost.iCloudRecordId
                  ofType:BKiCloudRecordTypeHosts];
      }
      ...
      [BKHosts saveHost:_bkHost.host
            withNewHost:_bkHost.iCloudConflictCopy.host
               hostName:_bkHost.iCloudConflictCopy.hostName
                sshPort:_bkHost.iCloudConflictCopy.port
                            ? _bkHost.iCloudConflictCopy.port.stringValue
                            : @""
      ...
      [BKHosts markHost:_bkHost.iCloudConflictCopy.host
              forRecord:[BKHosts recordFromHost:_bkHost]
           withConflict:NO];
      [[BKiCloudSyncHandler sharedHandler] checkForReachabilityAndSync:nil];
      [self.navigationController popViewControllerAnimated:YES];
    } else if (indexPath.row == 2) {
      [[BKiCloudSyncHandler sharedHandler]
          deleteRecord:_bkHost.iCloudConflictCopy.iCloudRecordId
                ofType:BKiCloudRecordTypeHosts];
      if (!_bkHost.iCloudRecordId) {
        [BKHosts markHost:_bkHost.iCloudConflictCopy.host
                forRecord:[BKHosts recordFromHost:_bkHost]
...
- (BOOL)showConflictSection {
  return (_bkHost.iCloudConflictDetected.boolValue &&
...
- (NSInteger)tableView:(UITableView *)tableView
  ...
  if (section == 1) { // Index number of interested section
    if (![self showConflictSection]) {
      ...
      return 3; // number of row in section when you click on show (if it's
                // higher than rows in Storyboard, app will crash)
    }
  } else {
    return [super tableView:tableView
        numberOfRowsInSection:section]; // keeps inalterate all other rows
  }
}
