...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if (pk.iCloudConflictDetected == [NSNumber numberWithBool:YES]) {
    if ((pk.iCloudConflictDetected.boolValue && pk.iCloudConflictCopy)) {
      ...
      [BKHosts markHost:pk.host
              forRecord:[BKHosts recordFromHost:pk]
           withConflict:NO];
    }
  } else {
    cell.textLabel.textColor = [UIColor labelColor];
...
- (void)tableView:(UITableView *)tableView
  ...
  if (editingStyle == UITableViewCellEditingStyleDelete) {
    CKRecordID *recordId =
        [[BKHosts.all objectAtIndex:indexPath.row] iCloudRecordId];
    if (recordId != nil) {
...
- (IBAction)unwindFromCreate:(UIStoryboardSegue *)sender {
  ...
  if (!details.isExistingHost) {
    NSIndexPath *newIdx = [NSIndexPath indexPathForRow:(BKHosts.count - 1)
                                             inSection:0];
    ...
    if ([self.tableView indexPathForSelectedRow] &&
        lastRow > [[self.tableView indexPathForSelectedRow] row]) {
      [self.tableView
          reloadRowsAtIndexPaths:@[ [[self tableView] indexPathForSelectedRow] ]
                withRowAnimation:UITableViewRowAnimationBottom];
    }
  }
}
