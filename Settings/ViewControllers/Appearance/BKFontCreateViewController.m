...
- (IBAction)importButtonClicked:(id)sender {
  ...
  if (fontUrl.length > 4 && [[fontUrl substringFromIndex:[fontUrl length] - 4]
                                isEqualToString:@".css"]) {
    if ([fontUrl rangeOfString:@"github.com"].location != NSNotFound &&
        [fontUrl rangeOfString:@"/raw/"].location == NSNotFound) {
      // Replace HTML versions of fonts with the raw version
      fontUrl = [fontUrl stringByReplacingOccurrencesOfString:@"/blob/"
                                                   withString:@"/raw/"];
    }
    ...
    UIAlertController *alertController = [UIAlertController
        alertControllerWithTitle:@"URL error"
                         message:
                             @"Fonts are assigned using valid .css files. "
                             @"Please open the gallery for more information."
                  preferredStyle:UIAlertControllerStyleAlert];
...
- (IBAction)didTapOnSave:(id)sender {
  if ([BKFont withName:self.nameTextField.text]) {
    ...
    NSError *error;
    [BKFont saveResource:self.nameTextField.text
