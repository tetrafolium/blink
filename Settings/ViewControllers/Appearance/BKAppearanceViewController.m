...
- (void)loadDefaultValues {
  ...
  if (selectedTheme != nil) {
    _selectedThemeIndexPath =
        [NSIndexPath indexPathForRow:[[BKTheme all] indexOfObject:selectedTheme]
                           inSection:BKAppearance_Themes];
  }
  ...
  if (selectedFont != nil) {
    ...
    _selectedFontIndexPath = [NSIndexPath indexPathForRow:row
                                                inSection:BKAppearance_Fonts];
  }
...
- (void)saveDefaultValues {
  if (_fontSizeField.text != nil &&
      ![_fontSizeField.text isEqualToString:@""]) {
    [BKDefaults
        setFontSize:[NSNumber numberWithInt:_fontSizeField.text.intValue]];
  }
  if (_selectedFontIndexPath != nil) {
    [BKDefaults
        setFontName:[[[BKFont all] objectAtIndex:_selectedFontIndexPath.row]
                        name]];
  }
  if (_selectedThemeIndexPath != nil) {
    [BKDefaults
        setThemeName:[[[BKTheme all] objectAtIndex:_selectedThemeIndexPath.row]
                         name]];
  }

  _keyboardStyleValue =
      [self _keyboardStyleFromIndex:_keyboardStyleSegmentedControl
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:BKAppearanceChanged
                                                      object:self];
}
...
- (void)setFontsUIForCell:(UITableViewCell *)cell
...
- (void)setThemesUIForCell:(UITableViewCell *)cell
               atIndexPath:(NSIndexPath *)indexPath {
...
- (CGFloat)tableView:(UITableView *)tableView
  ...
  UITableViewCell *cell =
      [tableView dequeueReusableCellWithIdentifier:
                     [self cellIdentifierForIndexPath:indexPath]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if (indexPath.section == BKAppearance_Terminal) {
    [self attachTestTerminalToView:cell.contentView];
             ...
             indexPath.section == BKAppearance_Fonts) {
    ...
    if ([BKDefaults selectedFontSize] != nil) {
      ...
      _fontSizeField.text =
          [NSString stringWithFormat:@"%@ px", [BKDefaults selectedFontSize]];
    } else {
      _fontSizeField.placeholder = @"";
    }
  } else if (indexPath.section == BKAppearance_FontSize && indexPath.row == 1) {
    _externalDisplayFontSizeField =
        [cell viewWithTag:EXTERNAL_DISPLAY_FONT_SIZE_FIELD_TAG];
    _externalDisplayFontSizeStepper =
        [cell viewWithTag:EXTERNAL_DISPLAY_FONT_SIZE_STEPPER_TAG];
    ...
    if (fontSize != nil) {
      ...
      _externalDisplayFontSizeField.text =
          [NSString stringWithFormat:@"%@ px", fontSize];
    } else {
  ...
  } else if (indexPath.section == BKAppearance_KeyboardAppearance &&
             indexPath.row == 0) {
    _keyboardStyleSegmentedControl = [cell viewWithTag:KEYBOARDSTYLE_TAG];
    _keyboardStyleSegmentedControl.selectedSegmentIndex =
        [self _keyboardStyleToIndex:_keyboardStyleValue];
  } else if (indexPath.section == BKAppearance_KeyboardAppearance &&
             indexPath.row == 1) {
    ...
    _defaultLayoutModeSegmentedControl.selectedSegmentIndex =
        [self _layoutModeToIndex:_defaultLayoutModeValue];
  } else if (indexPath.section == BKAppearance_Layout && indexPath.row == 1) {
    _overscanCompensationSegmentedControl =
        [cell viewWithTag:OVERSCAN_COMPENSATION_TAG];
    _overscanCompensationSegmentedControl.selectedSegmentIndex =
        [self _overscanCompensationToIndex:_overscanCompensationValue];
  }
  return cell;
}
...
- (NSInteger)_keyboardStyleToIndex:(BKKeyboardStyle)value {
  switch (value) {
  case BKKeyboardStyleSystem:
    return 0;
  case BKKeyboardStyleLight:
    return 1;
  case BKKeyboardStyleDark:
    return 2;
  default:
    return 0;
  }
  ...
  if (indexPath.section == BKAppearance_Themes) {
    if (indexPath.row == [[BKTheme all] count]) {
      ...
      if (_selectedThemeIndexPath != nil) {
        ...
        [[tableView cellForRowAtIndexPath:_selectedThemeIndexPath]
      ...
      BKTheme *theme =
          [[BKTheme all] objectAtIndex:_selectedThemeIndexPath.row];
    ...
    if (indexPath.row == [[BKFont all] count]) {
      ...
      if (_selectedFontIndexPath != nil) {
        // When in selectable mode, do not show details.
        [[tableView cellForRowAtIndexPath:_selectedFontIndexPath]
            setAccessoryType:UITableViewCellAccessoryNone];
      }
...
- (IBAction)unwindFromAddFont:(UIStoryboardSegue *)sender {
  ...
  if (![self.tableView
          cellForRowAtIndexPath:[NSIndexPath
                                    indexPathForRow:lastIndex
                                          inSection:BKAppearance_Fonts]]) {
    [self.tableView
        insertRowsAtIndexPaths:@[ [NSIndexPath
                                   indexPathForRow:lastIndex - 1
                                         inSection:BKAppearance_Fonts] ]
...
- (BOOL)tableView:(UITableView *)tableView
  ...
  if ((indexPath.section == BKAppearance_Themes &&
       indexPath.row >= [BKTheme defaultResourcesCount] &&
       indexPath.row < [BKTheme count]) ||
      (indexPath.section == BKAppearance_Fonts &&
       indexPath.row >= [BKFont defaultResourcesCount] &&
       indexPath.row < [BKFont count])) {
    return YES;
  } else {
    return NO;
  }
  ...
  if (editingStyle == UITableViewCellEditingStyleDelete) {
    ...
    if (indexPath.section == BKAppearance_Themes) {
      ...
      if (indexPath.row < _selectedThemeIndexPath.row) {
        _selectedThemeIndexPath =
            [NSIndexPath indexPathForRow:_selectedThemeIndexPath.row - 1
                               inSection:0];
      ...
      if (indexPath.row < _selectedFontIndexPath.row) {
        _selectedFontIndexPath =
            [NSIndexPath indexPathForRow:_selectedFontIndexPath.row - 1
                               inSection:0];
...
- (IBAction)stepperValueChanged:(id)sender {
  if (sender == _fontSizeStepper) {
    NSNumber *newSize =
        [NSNumber numberWithInteger:(int)[_fontSizeStepper value]];
    ...
    [_externalDisplayFontSizeField
        setText:[NSString stringWithFormat:@"%@ px", @(size)]];
  }
}
...
- (IBAction)defaultLayoutChanged:(UISegmentedControl *)sender {
  _defaultLayoutModeValue =
      [self _layoutModeFromIndex:sender.selectedSegmentIndex];
}

- (IBAction)overscanCompensationChanged:(UISegmentedControl *)sender {
  _overscanCompensationValue =
      [self _overscanCompensationFromIndex:sender.selectedSegmentIndex];
...
- (IBAction)alternateAppIconSwitchChanged:(id)sender {
  ...
  [[UIApplication sharedApplication] setAlternateIconName:appIcon
...
- (void)_writeColorShowcase {
  ...
  for (NSString *fg in fgs) {
    ...
    for (NSString *bg in bgs) {
      [line
          addObject:[NSString stringWithFormat:@" \033[%@\033[%@  gYw \033[0m",
                                               fg, bg]];
    }
