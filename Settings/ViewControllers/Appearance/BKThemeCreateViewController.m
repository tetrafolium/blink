...
- (IBAction)importButtonClicked:(id)sender {
  ...
  if (themeUrl.length > 4 &&
      [[themeUrl substringFromIndex:[themeUrl length] - 3]
          isEqualToString:@".js"]) {
    if ([themeUrl rangeOfString:@"github.com"].location != NSNotFound &&
        [themeUrl rangeOfString:@"/raw/"].location == NSNotFound) {
      // Replace HTML versions of themes with the raw version
      themeUrl = [themeUrl stringByReplacingOccurrencesOfString:@"/blob/"
                                                     withString:@"/raw/"];
    }
    [UIApplication sharedApplication].networkActivityIndicatorVisible = YES;
    self.urlTextField.enabled = NO;
    [self configureImportButtonForCancel];
    [BKSettingsFileDownloader
            downloadFileAtUrl:themeUrl
            expectedMIMETypes:@[ @"application/javascript", @"text/plain" ]
        withCompletionHandler:^(NSData *fileData, NSError *error) {
          if (error == nil) {
            [self performSelectorOnMainThread:@selector
                  (downloadCompletedWithFilePath:)
                                   withObject:fileData
                                waitUntilDone:NO];
          } else {
    ...
    UIAlertController *alertController = [UIAlertController
        alertControllerWithTitle:@"URL error"
                         message:
                             @"Themes must be .JS configuration files. Please "
                             @"open the gallery for more information."
                  preferredStyle:UIAlertControllerStyleAlert];
...
- (void)downloadCompletedWithFilePath:(NSData *)fileData {
  [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
  [self.importButton setTitle:@"Download Complete"
...
- (IBAction)didTapOnSave:(id)sender {
  if ([BKTheme withName:self.nameTextField.text]) {
    ...
    NSError *error;
    [BKTheme saveResource:self.nameTextField.text
              withContent:_tempFileData
                    error:&error];
...
- (void)showErrorMsg:(NSString *)errorMsg {
  UIAlertController *alertController =
      [UIAlertController alertControllerWithTitle:@"Themes error"
                                          message:errorMsg
                                   preferredStyle:UIAlertControllerStyleAlert];
  UIAlertAction *ok = [UIAlertAction actionWithTitle:@"OK"
                                               style:UIAlertActionStyleDefault
