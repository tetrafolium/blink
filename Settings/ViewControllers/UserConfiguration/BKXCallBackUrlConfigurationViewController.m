...
- (void)viewDidLoad {
  ...
  _validKeyRegexp = [[NSRegularExpression alloc] initWithPattern:@"[^a-zA-Z0-9]"
                                                         options:kNilOptions
                                                           error:nil];
  ...
  [_xCallbackUrlEnabledSwitch addTarget:self
                                 action:@selector(_onCallBackUrlEnabledChanged)
                       forControlEvents:UIControlEventValueChanged];

  [self.tableView registerClass:[UITableViewCell class]
         forCellReuseIdentifier:@"switch"];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if (indexPath.row == 0) {
    UITableViewCell *cell =
        [tableView dequeueReusableCellWithIdentifier:@"switch"
    ...
    UITableViewCell *cell =
        [tableView dequeueReusableCellWithIdentifier:@"URLKey"
                                        forIndexPath:indexPath];
...
- (BOOL)textField:(UITextField *)textField
  ...
  NSArray *matches =
      [_validKeyRegexp matchesInString:string
                               options:kNilOptions
...
- (NSString *)tableView:(UITableView *)tableView
  ...
  return [NSString
      stringWithFormat:@"Use x-callback-url for automation and inter-app "
                       @"communication. Your URL key should be kept "
                       @"secret.\n\nExample:\nblinkshell://run?key=%@&cmd=ls",
                       urlKey];
}
