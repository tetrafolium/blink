...
+ (NSString *)getInfoTypeFromDeviceName:(BKDeviceInfoType)type {
  ...
  NSArray *words =
      [deviceName componentsSeparatedByCharactersInSet:characterSet];
  ...
  for (NSString __strong *word in words) {
    if ([word length] <= 2)
      foundShortWord = true;
    if ([word containsString:@"iPhone"] || [word containsString:@"iPod"] ||
        [word containsString:@"iPad"] || [word containsString:@"Mac"]) {
      deviceType = word;
    } else if ([word length] > 2) {
      word = [word stringByReplacingCharactersInRange:NSMakeRange(0, 1)
                                           withString:[[word substringToIndex:1]
                                                          uppercaseString]];
  ...
  if (!foundShortWord && [names count] > 1) {
    ...
    if (lastChar == 's') {
      [names replaceObjectAtIndex:lastNameIndex
                       withObject:[name substringToIndex:[name length] - 1]];
    }
  }
