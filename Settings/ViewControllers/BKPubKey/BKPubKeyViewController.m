...
/*!
   ...
   @param importOrigin SshKeyImportOrigin origin of they key that's being
   imported
 */
- (void)_importKeyFromString:(NSString *)keyString
                importOrigin:(enum SshKeyImportOrigin)importOrigin {

  NSString *errorImportingKeyMessage =
      @"%origin% content couldn't be validated as a key";

  switch (importOrigin) {

  case FROM_CLIPBOARD:
    errorImportingKeyMessage = [errorImportingKeyMessage
        stringByReplacingOccurrencesOfString:@"%origin%"
                                  withString:@"Clipboard"];
    break;
  case FROM_FILE:
    errorImportingKeyMessage = [errorImportingKeyMessage
        stringByReplacingOccurrencesOfString:@"%origin%"
  ...
  [Pki importPrivateKey:keyString
             controller:self
            andCallback:^(Pki *key, NSString *comment) {
              if (key == nil) {
                UIAlertController *alertCtrl = [UIAlertController
                    alertControllerWithTitle:@"Invalid key"
                                     message:errorImportingKeyMessage
              ...
              BKPubKeyCreateViewController *ctrl =
                  [[BKPubKeyCreateViewController alloc]
  ...
  if (_selectable && indexPath.row == 0) {
    ...
    if ([cell.textLabel.text isEqual:@"id_rsa"]) {
      cell.detailTextLabel.text =
          [NSString stringWithFormat:@"Default Key - %@",
                                     [self fingerprint:pk.publicKey]];
    } else {
...
- (UITableViewCellEditingStyle)tableView:(UITableView *)tableView
  ...
  return _selectable ? UITableViewCellEditingStyleNone
...
- (IBAction)addKey:(id)sender {
  UIAlertController *keySourceController = [UIAlertController
      alertControllerWithTitle:@""
                       message:@""
                preferredStyle:UIAlertControllerStyleActionSheet];
  UIAlertAction *generate =
      [UIAlertAction actionWithTitle:@"Create New"
                               style:UIAlertActionStyleDefault
                             handler:^(UIAlertAction *_Nonnull action) {
                               [self createKey];
                             }];
  UIAlertAction *importFromClipboard =
      [UIAlertAction actionWithTitle:@"Import from clipboard"
                               style:UIAlertActionStyleDefault
                             handler:^(UIAlertAction *_Nonnull action) {
                               // ImportKey flow
  ...
  UIAlertAction *importFromFiles =
      [UIAlertAction actionWithTitle:@"Import from a file"
                               style:UIAlertActionStyleDefault
                             handler:^(UIAlertAction *_Nonnull action) {
                               // ImportKey flow
...
- (void)createKey {
  BKPubKeyCreateViewController *ctrl = [[BKPubKeyCreateViewController alloc]
      initWithStyle:UITableViewStyleGrouped];
  ctrl.createKeyDelegate = self;
  [self.navigationController pushViewController:ctrl animated:YES];
}
...
/*!
   @brief Call to open UIDocumentPicker so the user select the file that
   contains a key to be imported into the Secure Enclave
 */
  ...
  UIDocumentPickerViewController *documentPicker =
      [[UIDocumentPickerViewController alloc]
          initWithDocumentTypes:@[
            @"public.data", @"public.item", (NSString *)kUTTypeText
          ]
...
/*!
   @brief Import a key into the Secure Enclave getting the content from the
   clipboard
 */
...
- (void)viewControllerDidCreateKey:(BKPubKeyCreateViewController *)controller {
  ...
  [self.tableView insertRowsAtIndexPaths:@[ newIdx ]
...
- (IBAction)unwindFromDetails:(UIStoryboardSegue *)sender {
  ...
  [self.tableView reloadRowsAtIndexPaths:@[ _currentSelectionIdx ]
                        withRowAnimation:UITableViewRowAnimationNone];
}

// TODO: Maybe we should call it "markable", because the selection still exists
// and it is important.
...
- (void)tableView:(UITableView *)tableView
  ...
  if (_selectable && _currentSelectionIdx != nil) {
    ...
    [[tableView cellForRowAtIndexPath:_currentSelectionIdx]
...
- (id)selectedObject {
  ...
  return _selectable ? BKPubKey.all[_currentSelectionIdx.row - 1]
                     : BKPubKey.all[_currentSelectionIdx.row];
}
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller
  ...
  for (NSURL *fileUrl in urls) {
    ...
    NSString *keyString = [NSString stringWithContentsOfURL:fileUrl
