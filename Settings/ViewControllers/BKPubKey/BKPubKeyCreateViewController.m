...
- (void)viewDidLoad {
  ...
  _nameSection =
      @{HEADER : @"NAME", ROWS : @[ @(ROW_ID_NAME) ], FOOTER : _nameFooterText};
  _typeSection = @{
    HEADER : @"KEY",
  ...
  _infoSection = @{
    HEADER : @"INFORMATION",
    ROWS : @[],
    FOOTER : @"Blink creates PKCS#8 public and private keys, with AES 256 bit "
             @"encryption. Use \"ssh-copy-id [name]\" to copy the public key "
             @"to the server."
  };

  _keyTypeSegmentedControl = [[UISegmentedControl alloc] initWithItems:_types];
  [_keyTypeSegmentedControl addTarget:self
                               action:@selector(_keyTypeChanged:)
  ...
  [_nameTextField addTarget:self
                     action:@selector(_nameTextFieldChanged:)
  ...
  _commentsTextField.text =
      _comment
          ?: [NSString
                 stringWithFormat:@"%@@%@", [BKDefaults defaultUserName],
                                  [UIDevice getInfoTypeFromDeviceName:
                                                BKDeviceInfoTypeDeviceName]];

  _saveBarButtonItem = [[UIBarButtonItem alloc]
...
- (void)_saveBarButtonAction:(UIBarButtonItem *)sender {
  ...
  if ([BKPubKey withID:_nameTextField.text]) {
    ...
    if (!_importMode) {
      ...
      if (_bitsSegmentedControl.selectedSegmentIndex != NSNotFound) {
        if (_bitsSegmentedControl.selectedSegmentIndex >= 0) {
          bits = [_bitsValues[_bitsSegmentedControl.selectedSegmentIndex]
              intValue];
        }
      }
    ...
    _pubkey =
        [BKPubKey saveCard:_nameTextField.text
  ...
  if (errorMsg) {
    UIAlertController *alertController = [UIAlertController
        alertControllerWithTitle:@"Key error"
                         message:errorMsg
...
- (void)setKeyType:(const NSString *)type {
  ...
  [_nameFooterText
      replaceCharactersInRange:NSMakeRange(0, _nameFooterText.length)
                    withString:[NSString
                                   stringWithFormat:
                                       @"Default key must be named 'id_%@'",
  ...
  if ([BK_KEYTYPE_RSA isEqual:type]) {
    ...
    [rows addObject:@(ROW_ID_BITS)];
    footerText = @"For ECDSA keys size determines key length by selecting from "
...
- (void)setupBitsControl {
  ...
  for (NSNumber *bits in _bitsValues) {
    [_bitsSegmentedControl insertSegmentWithTitle:[bits stringValue]
                                          atIndex:n++
                                         animated:YES];
  }
