...
static void kbd_callback(const char *name, int name_len,
  ...
  for (int i = 0; i < num_prompts; i++) {
    ...
    NSString *text = [NSString stringWithCString:prompt.text
    ...
    responses[i].length =
        (int)[response lengthOfBytesUsingEncoding:NSUTF8StringEncoding];
  }
} /* kbd_callback */
...
- (int)main:(int)argc argv:(char **)argv {
  ...
  // Request tty can have different options, but depends on the process to
  // choose whether to use it or not. I want to maintain the REQUEST_TTY_XXX
  // because it will give us flexibility with the command in the future.
  if ((_options.request_tty == REQUEST_TTY_FORCE) ||
      (_options.request_tty == REQUEST_TTY_YES)) {
    _tty_flag = 1;
  } else {
    _tty_flag = 0;
  ...
  if ((addrs = [self resolve_host:_options.hostname
                             port:_options.port]) == NULL) {
  ...
  if (e != nil) {
    return [self
        dieMsg:[NSString stringWithFormat:@"Could not connect to host: %@",
                                          [e localizedDescription]]];
  }

  // Login to the host through one of the possible identities
  [self load_identity_files];
  [self ssh_login:_identities
               to:(struct sockaddr *)&hostaddr
             port:_options.port
             user:_options.user
          timeout:_options.connection_timeout
            error:&e];
  ...
  [self debugMsg:[NSString stringWithFormat:@"session finished with code %d",
                                            exit_code]];
...
- (void)processHostSettings {
  ...
  if (!(host = [BKHosts
            withHost:[NSString stringWithUTF8String:_options.hostname]])) {
    return;
  }

  _options.hostname =
      host.hostName ? [host.hostName UTF8String] : _options.hostname;
  ...
  _options.identity_file =
      _options.identity_file ? _options.identity_file : [host.key UTF8String];
...
- (void)load_identity_files {
  ...
  if (_options.identity_file) {
    if ((pk = [BKPubKey
             withID:[NSString stringWithUTF8String:_options.identity_file]]) !=
        nil) {
...
- (struct addrinfo *)resolve_host:(const char *)name port:(int)port {
  ...
  // IPv4 / IPv6
  hints.ai_family =
      _options.address_family == -1 ? AF_UNSPEC : _options.address_family;
...
- (void)ssh_connect:(const char *)host
              addrs:(struct addrinfo *)aitop
          succ_addr:(struct sockaddr_storage *)hostaddr
              error:(NSError **)error {
  struct addrinfo *ai;
  ...
  for (ai = aitop; ai; ai = ai->ai_next) {
    ...
    [self
        debugMsg:[NSString
                     stringWithFormat:@"Connecting to %.200s [%.100s] port %s.",
                                      host, ntop, strport]];
    ...
    if (_sock < 0) {
      ...
      if (!ai->ai_next) {
        *error = [NSError
            errorWithDomain:@"blk.ssh.libssh2"
                       code:-1
                   userInfo:@{
    ...
    if ([self timeout_connect:_sock
      ...
      [self
          debugMsg:[NSString stringWithFormat:@"connect to host %s port %s: %s",
        ...
        errorWithDomain:@"blk.ssh.libssh2"
               ...
               userInfo:@{
                 NSLocalizedDescriptionKey : [NSString
                     stringWithFormat:@"ssh: connect to host %s port %s: %s",
                                      host, strport, strerror(errno)]
               }];
  ...
  if (0 != [self ssh_set_session]) {
    *error = [NSError
        errorWithDomain:@"blk.ssh.libssh2"
                   code:-1
               userInfo:@{
                 NSLocalizedDescriptionKey : @"Error establishing SSH session."
               }];
    return;
  }
    ...
    if (![self verify_host:ntop]) {
      *error = [NSError
          errorWithDomain:@"blk.ssh.libssh2"
                     code:-1
                 userInfo:@{
                   NSLocalizedDescriptionKey : @"Host key verification failed."
                 }];
    }
  } else {
...
- (int)ssh_set_session {
  ...
  if (!_session) {
    ...
    // *error = [NSError errorWithDomain:@"bnk.sessions.sshwrapper" code:401
    // userInfo:@{NSLocalizedDescriptionKey : @"Create session failed"}];
    return -1;
  }
    ...
    if (!userauthlist) {
      if (libssh2_session_last_errno(_session) != LIBSSH2_ERROR_EAGAIN) {
        *error = [NSError
            errorWithDomain:@"blk.ssh.libssh2"
                       code:-1
                   userInfo:@{NSLocalizedDescriptionKey : @"No userauth list"}];
        return;
      } else {
  ...
  if (!succ) {
    *error = [NSError
        errorWithDomain:@"blk.ssh.libssh2"
               ...
               userInfo:@{
                 NSLocalizedDescriptionKey : [NSString
                     stringWithFormat:@"Permission denied (%s).", userauthlist]
               }];
    return;
  }
  ...
  do {
    ...
    if (!password) {
      NSString *prompt = [NSString
          stringWithFormat:@"%s@%s's password: ", user, _options.hostname];
    ...
    if (strlen(password) != 0) {
      while ((rc = libssh2_userauth_password(_session, user, password)) ==
             LIBSSH2_ERROR_EAGAIN)
        ;
      if (rc == 0) {
        ...
        libssh2_session_last_error(_session, &errmsg, NULL, 0);
        [self errMsg:[NSString
                         stringWithFormat:
                             @"Authentication by password failed: %s", errmsg]];
        return 0;
      }
    }
  } while (--retries);
...
- (int)ssh_login_publickey:(const char *)user {
  ...
  for (BKPubKey *pk in _identities) {
    ...
    if ([pk isEncrypted]) {
      //      [_device setSecureTextEntry:YES];
      NSString *prompt = [NSString
          stringWithFormat:@"Enter your passphrase for key '%@':", pk.ID];
    ...
    while ((rc = libssh2_userauth_publickey_frommemory(
                _session, user, strlen(user), pub,
                strlen(pub), // or sizeof_publickey methods
                priv, strlen(priv), passphrase.UTF8String)) ==
           LIBSSH2_ERROR_EAGAIN)
      ;
    if (rc == 0) {
      [self debugMsg:@"Authentication succeeded."];
...
- (int)timeout_connect:(int)fd
             serv_addr:(const struct sockaddr *)addr
...
- (int)set_nonblock:(int)fd {
  ...
  if ((arg = fcntl(fd, F_GETFL, NULL)) < 0) {
    [self debugMsg:[NSString stringWithFormat:@"Error fcntl(..., F_GETFL) (%s)",
                                              strerror(errno)]];
    return -1;
  }
  arg |= O_NONBLOCK;
...
- (int)ssh_session_start {
  // This function is responsible to start all the session requirements, like
  // port forwarding, shell, commands... using the ssh library that we want to.

  // It would be responsible to open the channels, and requesting the shells or
  // commands as necessary We just have a couple of things, so maybe we can
  // avoid it. [self ssh_session_setup];

  // It would be responsible to read / write to the channels, shutting down the
  // system and dropping the result of the operation.
  int rc = 0;
  char *errmsg;

  while ((_channel = libssh2_channel_open_session(_session)) == NULL &&
         libssh2_session_last_error(_session, NULL, NULL, 0) ==
             LIBSSH2_ERROR_EAGAIN) {
  ...
  if (_channel == NULL) {
    libssh2_session_last_error(_session, &errmsg, NULL, 0);
    [self
        debugMsg:[NSString stringWithFormat:
                               @"ssh_session_start: error creating channel: %s",
                               errmsg]];
    return -1;
  }
  ...
  if (_tty_flag) {
    // [self debugMsg:[NSString stringWithFormat:@"Sending env LC_CTYPE =
    // UTF-8"]]; while ((rc = libssh2_channel_setenv(_channel, "LC_CTYPE",
    // "UTF-8")) == LIBSSH2_ERROR_EAGAIN) {
    //   waitsocket(_sock, _session);
    // }

    while ((rc = libssh2_channel_request_pty(_channel, TERM)) ==
           LIBSSH2_ERROR_EAGAIN) {
      waitsocket(_sock, _session);
    }
    if (rc) {
      libssh2_session_last_error(_session, &errmsg, NULL, 0);
      [self debugMsg:[NSString
                         stringWithFormat:
                             @"ssh_session_start: error creating channel: %s",
                             errmsg]];
      return -1;
    ...
    libssh2_channel_request_pty_size(_channel, _device->win.ws_col,
                                     _device->win.ws_row);
  }
  ...
  if (_command != NULL) {
    //  else {
    [self
        debugMsg:[NSString stringWithFormat:@"sending command: %s", _command]];
    while ((rc = libssh2_channel_exec(_channel, _command)) ==
           LIBSSH2_ERROR_EAGAIN) {
      waitsocket(_sock, _session);
    }
    if (rc != 0) {
      libssh2_session_last_error(_session, &errmsg, NULL, 0);
      [self debugMsg:[NSString
                         stringWithFormat:@"ssh_session_start: error exec: %s",
                                          errmsg]];
      return -1;
    }
    ...
    if (rc) {
      ...
      [self
          debugMsg:[NSString
                       stringWithFormat:@"ssh_session_start: error shell : %s",
                                        errmsg]];
      return -1;
    }
...
- (int)verify_host:(char *)addr {
  ...
  //  NSURL *dd = [[[NSFileManager defaultManager]
  //  URLsForDirectory:NSDocumentDirectory inDomains:NSUserDomainMask]
  //  firstObject]; NSURL *khURL = [dd
  //  URLByAppendingPathComponent:@"known_hosts"];
  const char *khFilePath =
      [BlinkPaths knownHostsFile].UTF8String; // [khURL.path UTF8String];
  ...
  if (key) {
    ...
    int check = libssh2_knownhost_checkp(
        kh, _options.hostname, _options.port, key, key_len,
    ...
    if (check == LIBSSH2_KNOWNHOST_CHECK_FAILURE) {
      ...
      [self
          errMsg:[NSString stringWithFormat:@"The authenticity of host %.200s "
                                            @"(%s) can't be established.\r\n"
                                             "%s key fingerprint is %@",
                                            _options.hostname, addr, type_str,
                                            fingerprint]];
      ...
      [self
          errMsg:[NSString stringWithFormat:
                               ...
                               @"@@@@@@\r\n"
                                ...
                                "This might be due to someone doing something "
                                "nasty or just a change in the host.\r\n"
                                "Current %s key fingerprint is %@",
                               type_str, _options.hostname, addr, type_str,
                               fingerprint]];
  ...
  if (!succ &&
      (succ = [self
           confirm:"Are you sure you want to continue connecting (yes/no)?"])) {
    [self authorize_new_key:key
                     length:key_len
                       type:kh_key_type
                 knownHosts:kh
                   filePath:khFilePath];
...
- (int)authorize_new_key:(const char *)key
                  length:(ssize_t)key_len
                    type:(int)key_type
              knownHosts:(LIBSSH2_KNOWNHOSTS *)kh
                filePath:(const char *)kh_path {
  int rc;

  // Add key to the server
  rc = libssh2_knownhost_addc(kh, _options.hostname,
                              NULL,                  // No hashed addr, no salt
                              key, key_len, NULL, 0, // No comment
  ...
  if (rc < 0) {
    ...
    libssh2_session_last_error(_session, &errmsg, NULL, 0);
    [self
        errMsg:[NSString stringWithFormat:@"Error adding to the known host: %s",
                                          errmsg]];
  }
  ...
  if (rc < 0) {
    ...
    libssh2_session_last_error(_session, &errmsg, NULL, 0);
    [self errMsg:[NSString
                     stringWithFormat:@"Error writing known host: %s", errmsg]];
  } else {
    [self
        errMsg:[NSString
                   stringWithFormat:
                       @"Permanently added key for %s to list of known hosts.",
                       _options.hostname]];
  }
  return 1;
}
