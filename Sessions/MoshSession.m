...
static const char *usage_format =
    ...
    "-T                                   do not allocate a pseudo tty on ssh "
    "connection\r\n"
    "-2                                   use ssh2 command\r\n"
    "-I id                                ssh authentication identity name\r\n"
    //  "        --ssh=COMMAND        ssh command to run when setting up
    //  session\r\n" "                                (example: \"ssh -p
    //  2222\")\r\n" "                                (default: \"ssh\")\r\n"
    "\r\n"
    ...
    "                --help               this message\r\n"
    "\r\n"
    " --experimental-remote-ip={remote|local}    method used to discover the "
    "IP address that the mosh-client connects to \r\n"
    "                                            (default: local)\r\n"
    "\r\n";
...
- (int)initParamaters:(int)argc argv:(char **)argv {
  ...
  while (1) {
    int option_index = 0;
    int c =
        getopt_long(argc, argv, "anp:I:P:k:T2", long_options, &option_index);
    if (c == -1) {
      break;
    }
    ...
    case '2':
      ...
      break;
    case 'r':
      self.sessionParams.predictionMode =
          [NSString stringWithFormat:@"%s", optarg];
      break;
    case 'p':
    ...
    case 'T':
      ...
      break;
    case 'R':
      self.sessionParams.experimentalRemoteIp =
          [NSString stringWithFormat:@"%s", optarg];
      break;
    default:
  ...
  if (![@[ @"remote", @"local" ]
          containsObject:self.sessionParams.experimentalRemoteIp]) {
    return [self dieMsg:@(usage_format)];
  }
  ...
  if (idx_remote_command) {
    for (int i = 0; i < idx_remote_command; i++) {
      [remoteCmdChunks
          addObject:[NSString stringWithFormat:@"%s", remote_command[i]]];
    }
    self.sessionParams.startupCmd =
        [remoteCmdChunks componentsJoinedByString:@" "];
  }
  ...
  if (self.sessionParams.key) {
    ...
    NSString *moshServerCmd =
        [self getMoshServerStringCmd:self.sessionParams.serverPath
    ...
    NSError *error;
    if (useSSH2) {
      [self setConnParamsWithSsh2:ssh
                         userHost:userhost
                             port:sshPort
                         identity:sshIdentity
                      moshCommand:moshServerCmd
                            error:&error];
    } else {
      [self setConnParamsWithSsh:ssh
                        userHost:userhost
                            port:sshPort
                        identity:sshIdentity
                          sshTTY:sshTTY
  ...
  self.sessionParams.predictionMode =
      self.sessionParams.predictionMode ?: @"adaptive";
  if ([@[ @"always", @"adaptive", @"never" ]
          indexOfObject:self.sessionParams.predictionMode] == NSNotFound) {
    return [self
        dieMsg:@"Unknown prediction mode. Use one of: always, adaptive, never"];
  }
  return 0;
}
...
- (int)main:(int)argc argv:(char **)argv {
  ...
  NSString *locales_path = [[NSBundle mainBundle] pathForResource:@"locales"
                                                           ofType:@"bundle"];
  ...
  mosh_main(_stream.in, _stream.out, &_device->win, &__state_callback,
            (void *)_selfRef, [self.sessionParams.ip UTF8String],
            [self.sessionParams.port UTF8String],
            [self.sessionParams.key UTF8String],
            [self.sessionParams.predictionMode UTF8String], encodedState.bytes,
            encodedState.length);
...
- (void)processMoshSettings:(BKHosts *)host {
  ...
  if (!self.sessionParams.port && host.moshPort) {
    ...
    if (host.moshPortEnd) {
      self.sessionParams.port =
          [NSString stringWithFormat:@"%@:%@", host.moshPort, host.moshPortEnd];
    }
  }
  //  self.sessionParams.port = self.sessionParams.port ?: [host.moshPort
  //  stringValue];
  ...
  NSString *predictionMode =
      host.prediction ? predictionModeStrings[host.prediction] : nil;
  self.sessionParams.predictionMode =
      self.sessionParams.predictionMode ?: predictionMode;
}
...
- (NSString *)getMoshServerStringCmd:(NSString *)server
  ...
  // Prepare ssh command
  NSMutableArray *moshServerArgs =
      [NSMutableArray arrayWithObjects:server, @"new", @"-s", @"-c", colors,
                                       @"-l LC_ALL=en_US.UTF-8", nil];
  if (port) {
  ...
  return [NSString
      stringWithFormat:@"%@", [moshServerArgs componentsJoinedByString:@" "]];
}
...
- (void)setConnParamsWithSsh:(NSString *)ssh
  ...
  BOOL useIPFromSSHConnectionEnv =
      [@"remote" isEqual:self.sessionParams.experimentalRemoteIp];
  if (useIPFromSSHConnectionEnv) {
    NSString *echoSshConnectionCommand =
        @"echo \"\" && echo \"MOSH SSH_CONNECTION $SSH_CONNECTION\" &&";
    sshArgs = [@[
      ssh, @"-o compression=no", sshTTY ? @"-t" : @"-T", userHost,
      echoSshConnectionCommand, command
    ] mutableCopy];
  } else {
    sshArgs = [@[
      ssh, @"-o _printaddress=yes", @"-o compression=no",
      sshTTY ? @"-t" : @"-T", userHost, command
    ] mutableCopy];
  }
  ...
  if (identity) {
    [sshArgs insertObject:[NSString stringWithFormat:@"-i %@", identity]
                  atIndex:1];
  }
  if (_debug) {
  ...
  NSRegularExpression *ipFormat =
      [NSRegularExpression regularExpressionWithPattern:ipPattern
                                                options:0
                                                  error:nil];

  NSString *connPattern = @"MOSH CONNECT (\\d+) (\\S*)$";
  NSRegularExpression *connFormat =
      [NSRegularExpression regularExpressionWithPattern:connPattern
                                                options:0
                                                  error:nil];
  ...
  while ((n = getline(&buf, &buf_sz, term_r)) >= 0) {
    ...
    if (match) {
      self.sessionParams.ip =
          [line substringWithRange:[match rangeAtIndex:ipMatchIdx]];
      continue;
    }
  ...
  if (!self.sessionParams.ip) {
    *error = [NSError
        errorWithDomain:@"blink.mosh.ssh"
                   code:0
               userInfo:@{
                 NSLocalizedDescriptionKey : @"Did not find remote IP address"
               }];
    return;
  }
...
- (void)setConnParamsWithSsh2:(NSString *)ssh
                     userHost:(NSString *)userHost
                         port:(NSString *)port
                     identity:(NSString *)identity
                  moshCommand:(NSString *)command
  ...
  NSMutableArray *sshArgs = [NSMutableArray
      arrayWithObjects:ssh, @"-t", userHost, @"--", command, nil];
  if (port) {
    [sshArgs insertObject:[NSString stringWithFormat:@"-p %@", port] atIndex:1];
  }
  if (identity) {
  ...
  SSHSession *sshSession = [[SSHSession alloc] initWithDevice:_device
                                                    andParams:nil];
...
- (void)kill {
  // MOSH-ESC .
  [_device write:[NSString
                     stringWithFormat:@"%@%@", _escapeKey ?: @"\x1e", @"\x2e"]];
...
- (void)suspend {
  ...
  // MOSH-ESC C-z
  [_device write:[NSString
                     stringWithFormat:@"%@%@", _escapeKey ?: @"\x1e", @"\x1a"]];
