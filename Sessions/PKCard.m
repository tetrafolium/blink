...
- (NSString *)privateKeyWithPassphrase:(NSString *)passphrase {
  ...
  if (!PEM_write_bio_PKCS8PrivateKey(fpem, _pkey, cipher,
  ...
  NSString *key = [[NSString alloc] initWithBytes:pkey
                                           length:sz
...
+ (void)loadIDS {
  if (DocumentsDirectory == nil) {
    ...
    DocumentsDirectory = [[[NSFileManager defaultManager]
  ...
  // Load IDs from file
  if ((Identities = [NSKeyedUnarchiver unarchiveObjectWithFile:KeysURL.path]) ==
      nil) {
...
+ (id)saveCard:(NSString *)ID
  ...
  if (!card) {
    card = [[PKCard alloc] initWithID:ID
