...
- (id)initWithDevice:(TermDevice *)device andParams:(MCPParams *)params {
  if (self = [super initWithDevice:device andParams:params]) {
    ...
    _cmdQueue =
        dispatch_queue_create("mcp.command.queue", DISPATCH_QUEUE_SERIAL);

    [self setActiveSession];
    //    ios_setMiniRoot([BlinkPaths documents]);
    //    [self updateAllowedPaths];
    //    [[NSFileManager defaultManager] changeCurrentDirectoryPath:[BlinkPaths
    //    documents]]; ios_setContext((__bridge void*)self);
    //
    thread_stdout = nil;
    thread_stdin = nil;
    thread_stderr = nil;
...
- (void)executeWithArgs:(NSString *)args {
  dispatch_async(_cmdQueue, ^{
    ...
    [[NSFileManager defaultManager]
        changeCurrentDirectoryPath:[BlinkPaths documents]];
    ...
    if ([@"mosh" isEqualToString:self.sessionParams.childSessionType] &&
        self.sessionParams.hasEncodedState) {
      _childSession = [[MoshSession alloc]
          initWithDevice:_device
               andParams:self.sessionParams.childSessionParams];
...
/*!
   @brief Enqueue a new command coming from a x-callback-url. After completing
   successfully return to the
   @discussion Accepts the x-callback-url and the x-success URL to call after a
   successful command completion
   @param cmd Command to be executed
   @param xCallbackSuccessUrl Success URL of the original application (like
   Shortcuts) to return to after reunning the command
 */
...
- (NSArray<NSString *> *)_symlinksInHomeDirectory {
  ...
  NSArray<NSString *> *files = [fm contentsOfDirectoryAtPath:documentsPath
                                                       error:nil];
  ...
  for (NSString *path in files) {
    ...
    NSString *destPath = [fm destinationOfSymbolicLinkAtPath:filePath
                                                       error:nil];
    if (!destPath) {
      continue;
    }
...
- (void)_runSSHCopyIDWithArgs:(NSString *)args {
  ...
  _childSession = [[SSHCopyIDSession alloc]
      initWithDevice:_device
           andParams:self.sessionParams.childSessionParams];
...
- (void)_runSSHWithArgs:(NSString *)args {
  self.sessionParams.childSessionParams = nil;
  _childSession =
      [[SSHSession alloc] initWithDevice:_device
                               andParams:self.sessionParams.childSessionParams];
